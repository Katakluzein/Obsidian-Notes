...
这种方法利用了“恒等映射在同调上诱导同构”这一函子性核心，避免了复杂的链级计算。

既然你已经通过三种不同的视角（链群定义、相对循环、长正合序列）理解了为什么 $H_n(X, X)=0$，是否想尝试用这种“长正合序列缩减法”来证明更具挑战性的 **$H_n(X, A) \cong \tilde{H}_n(X/A)$**（当 $(X, A)$ 为优良对时）？