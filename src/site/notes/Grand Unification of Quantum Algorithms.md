...
$$
(21)
通过检验可以看出，只要特征值$\lambda$的尺度合理，$U^\dagger U = I$，满足$U$的幺正性要求。虽然一个通用的块编码不必采取这种特殊形式，但这种编码方式对于我们的说明性目的已经足够了。此外，关于通用块编码的处理在文献[8,12]中有介绍，其中指出，一个通用的块编码在一个与$H$的本征基相关的特殊基中，会呈现出与公式(20)相似的形式。
