...
>
>   (ii) 如果 $V=W$ 并且 $\varphi=\psi$, 则 $\sigma=\lambda \mathbb{1}_V$, 其中 $\lambda$ 是 $K$ 中某个元素.
>   \newline
>   使用矩阵的语言重述。
>   \newline
...
>
>   (ii) 如果 $\Phi=\Psi$, 则 $C=\lambda \mathbb{1}$, 其中 $\lambda$ 是某个复数.
...
$$
>   因此$\psi(g)x\in V_\lambda ,~\forall g\in G$，由于不可约性，$V_\lambda=V$，$\sigma =\lambda \mathbb{1}$。
...
>
$$
\frac{1}{|G|}W^\dagger W D=\mathbb{1}
$$
>   写成分量形式
>
$$
