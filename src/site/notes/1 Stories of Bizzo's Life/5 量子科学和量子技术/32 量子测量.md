...
设 $\rho_\theta = \mathcal{E}_\theta(\rho_0)$，$2\partial_\theta \rho_\theta = L_\theta \rho_\theta + \rho_\theta L_\theta$。$\mathcal{F}[\rho_\theta]=\mathrm{tr}(\rho_\theta L_\theta^2)$，简记 $\rho = \rho_\theta$, $L=L_\theta$。我们的目标是 $\max_{\rho_0} \mathcal{F}[\rho_\theta]$。
关于$\delta \rho_0$变分，
$$
