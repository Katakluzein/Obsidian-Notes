...
## 经典测量理论
### 状态、命题和物理量
在经典测量理论中，物理系统是概率空间、状态是概率测度、命题是事件、物理量是随机变量（[[Moretti2017\|Moretti2017]] Sec.7.2）。 以经典力学为例，一个$2n$自由度的哈密顿量系统栖息在一个纤维丛上，全空间是
$$
\mathscr{H}_{n+1}=\bigsqcup_{t \in \mathbb{R}} \mathscr{F}_{t}
$$
其中相空间$\mathscr{F}_{t}$是作为纤维的$2n$维的辛流形（[[1 Stories of Bizzo's Life/3 引力理论 宇宙学 天文学/11 微分几何#辛几何\|11 微分几何#辛几何]]），底空间是时间轴$\mathbb{R}$。给定时刻$t$，物理系统是一个概率空间$(\mathscr{F}_{t},\mathscr{B}\left(\mathscr{F}_{t}\right),v_{t})$，其中$\mathscr{F}_{t}$是样本空间，$\mathscr{B}\left(\mathscr{F}_{t}\right)$是事件空间，$v_t$是概率测度。确定性系统和概率性系统分别用相空间中的点$r(t) \in \mathscr{F}_{t}$和相空间中的概率密度函数$\rho_{t}(s)$描述。这两种态都可以被视为相空间$\mathscr{F}_{t}$上的 Borel 概率测度$\left\{v_{t}\right\}_{t \in \mathbb{R}}$
...
$$
相当于在说，“在$t$时刻，系统的物理量的取值落在$X$中”。由于$f$是可测函数，因此$f^{-1}(X)\in \mathscr{B}(\mathscr{F_t})$，我们可以衡量该命题的真值（即，测量该集合的大小），仍用$v_{t}\left(P_{X}^{(f)}\right)$表示。
...
>   进一步有 $$-\int_{-\infty}^{\infty} p(x|\phi) \mathrm{d} z+\int_{-\infty}^{\infty}\frac{\partial p(x|\phi)}{\partial \phi}(\hat{\phi}-\phi) \mathrm{d} z=0.$$
...
可以用POVM来描述对微观系统的测量过程。根据Naimark膨胀(dilation)定理可知，小系统的POVM实际上是以它为子系统的更大的系统的PVM。要对系统$A$上的密度算符$\rho_A$做POVM测量$\left\{F_{i}\right\}_{i=1}^{n}$，其中$\sum_{i=1}^{n} F_{i}=\mathbb{I}_A$，$0<E_i\leqslant \mathbb{I}$，可以先引入辅助系统(探针)$B$，制备直积态$\rho_A\otimes|0\rangle\langle 0|_{B}$，对整个系统做幺正演化$U:\mathsf{H}_{AB}\rightarrow \mathsf{H}_{AB}$，然后对辅助系统$B$做投影测量（也可以视为对整个系统$AB$的投影测量。）
$$\{\Pi_i=|i\rangle\left\langle\left. i\right|_{B}\right\}_{i=1}^{n}$$
...
$$
$W$是系统$A$的任意给定幺正算符。根据上述定义可知$V:\mathsf{H}_{A}\rightarrow \mathsf{H}_{AB}$ 是一个 isometry，即$V^\text{} V=\mathbb{I}_A$，上述条件并不足以唯一地确定$U$，因为我们还没有确定$U$在$|e_i\rangle_A\otimes |i\rangle_B,~i>1$这些基矢上的行为，我们可以将$V$拓展到整个系统，成为幺正算符$U$。容易验证，两种测量方式给出的测得第$i$个结果的概率相同，即
$$
...
$$
其中$M_{i}=W \sqrt{F_{i}}$称为**测量算符** (measurement operator)，Wiseman将$F_i$称为**概率算符**(probability operator)或者**effect**（ [[wiseman2009book\|wiseman2009book]] Sec.1.2.5，更一般地，任何满足 $0\leqslant A \leqslant \mathbb{I}$ 的算符$A$都称为effect)，也可以成为POVM元 (element)。从上述结果可知，测量结果并不完全由POVM确定，还与$W$的选择有关。
...
在更一般的情况下，POVM被推广为仪器 $\mathcal{J}=\{ \mathcal{M}_r\}$，每个测量结果$r$与一个量子操作(参见：定义\ref{def:quantumchannel}中的量子操作(quantum operation)，态空间上全正但不必保迹的凸线性映射)  $\mathcal{M}_{r}$，而不是一个POVM元相对应，
$$
...
$$
其中$\mathcal{M}\equiv \sum_r \mathcal{M}_r=\sum_{r, j} \mathcal{J}\left[\hat{\Omega}_{r, j}\right]$是量子信道。从概率分布的角度看，任何仪器给出的概率分布都和某个POVM给出的概率分布相同，因此允许使用仪器不会使得测量结果的概率分布有更高的Fisher信息量。

态区分为目标的POVM的例子(参见：[SE帖](https://quantumcomputing.stackexchange.com/questions/13426/what-are-examples-of-non-trivial-povm-measurements))
$$
...
根据[[Efficient computation of the Nagaoka–Hayashi bound for multiparameter estimation with separable measurements\|Efficient computation of the Nagaoka–Hayashi bound for multiparameter estimation with separable measurements]]，量子精密测量有很多应用。这一点在引力波探测中表现得尤为明显，将压缩光注入LIGO后，其灵敏度得到了显著提升³。此外，量子资源在诸多应用领域都展现了出色的测量能力，例如光学干涉测量4-8、量子超分辨率9,10、量子增强相位跟踪11,12以及量子定位13,14等。
...
量子测量与量子纠错、量子控制和开放系统理论是深度交融的。开放系统理论为研究探针的动力学演化提供了基本框架。量子控制和纠错理论在原有的领域中的主要目标是对抗退相干和噪声，在量子精密计量中，控制和纠错的目标得到了延伸，即尽可能地提高测量精度。
...
参考[[Multi-parameter estimation in critical quantum metrology\|Multi-parameter estimation in critical quantum metrology]]。设正定算符集$\{\Pi_b\}$构成一个POVM，$\rho_\theta$测量所得的概率分布记为$p(b,\theta)=\mathrm{tr}(\rho_\theta \Pi_b)$，经典Fisher信息量
$$
...
$$
eq-eq-convexofQFI
这导致最优初态一定在边界（extreme point 极限点 纯态）上取得。
...
待整理：pjt.p.39，被表述为HNKS{（[[zhou2021ErrorcorrectedQuantumMetrology\|zhou2021ErrorcorrectedQuantumMetrology]] (2.74)）} 0
[[2024-04-18-20-07-13.png|300]]
...
fig:optimizationinhermite
[[2024-04-18-09-14-34.png|300]]
最先进的bound在[[Using adaptiveness and causal superpositions against noise in quantum metrology\|Using adaptiveness and causal superpositions against noise in quantum metrology]]，他们证明了因果叠加CS和AD相较E没有优势，串联和并联以及因果叠加在渐进情况下是一样好的。

计算这个上界的数值方法：

$$
\begin{aligned}
    & \mathcal{F}_N^{\text{Para}}(\mathcal{E}_\varphi^{\otimes N}) \le 4 \min_{\{K_i\}} \left[ N\|\alpha\| + N(N-1)\|\beta\|^2 \right] \\
    =&~ 4 \min_{h=h^\dagger} \left[ N\|\alpha(h)\| + N(N-1)\|\beta(h)\|^2 \right] \\
    =&~ 4 \min_{h, t_\alpha, t_\beta} \left[ N t_\alpha + N(N-1) t_\beta \right] \\
    & \quad \text{s.t.} \quad \|\alpha(h)\| \le t_\alpha, \quad \|\beta(h)\|^2 \le t_\beta \\
    =&~ 4 \min_{h, t_\alpha, t_\beta} \left[ N t_\alpha + N(N-1) t_\beta \right] \\
    & \quad \text{s.t.} \quad t_\alpha \mathbb{I} - \alpha(h) \succeq 0, \quad t_\beta \mathbb{I} - \beta(h)^\dagger \beta(h) \succeq 0 \\
    =&~ 4 \min_{h, t_\alpha, t_\beta} \left[ N t_\alpha + N(N-1) t_\beta \right] \\
    & \quad \text{s.t.} \quad \begin{pmatrix} t_\alpha \mathbb{I} & \dot{\mathbf{K}}(h)^\dagger \\ \dot{\mathbf{K}}(h) & \mathbb{I} \end{pmatrix} \succeq 0, \quad \begin{pmatrix} t_\beta \mathbb{I} & \beta(h)^\dagger \\ \beta(h) & \mathbb{I} \end{pmatrix} \succeq 0
\end{aligned}
$$
最后一步将问题完全转化为了一个标准的SDP问题。程序参见：ICO策略下不同QFI界的比较.nb

### 信道估计的最优初态

设 $\rho_\theta = \mathcal{E}_\theta(\rho_0)$，$2\partial_\theta \rho_\theta = L_\theta \rho_\theta + \rho_\theta L_\theta$。$\mathcal{F}[\rho_\theta]=\mathrm{tr}(\rho_\theta L_\theta^2)$，简记 $\rho = \rho_\theta$, $L=L_\theta$。我们的目标是 $\max_{\rho_0} \mathcal{F}[\rho_\theta]$。我们在最大化一个凸函数 ( 搜：Fisher信息量具有凸性 )，这通常是非常困难的。关于$\delta \rho_0$变分，
$$
2\left(\delta \partial_\theta \rho\right)=L(\delta \rho)+\rho(\delta L)+(\delta L) \rho+(\delta \rho) L
$$
无论使用何种替换方式\footnote{信道估计的最优初态的变分公式.nb}，都能消去$\delta \mathcal{F}$中的$\delta L$，得到
$$
\delta\mathcal{F} = \operatorname{Tr} \left[ \delta \rho_0 \left( 2(\partial_\theta \mathcal{E}_\theta)^\dagger(L) - \mathcal{E}_\theta^\dagger(L^2) \right) \right]
$$
这是一个关于 $\rho_0$ 和 $L$ 的泛函。反过来，如果我们从$\delta \mathcal{F}$中消去$\delta \rho_0$，只保留$\delta L$，==得到什么？==

根据QFI在密度矩阵集合上的凸性 ，最优初态是纯态。我们构建拉格朗日函数以求解在约束 $\rho_0 \in \mathcal{D}(\mathcal{H})$ 下的极值问题：
$$
\mathcal{L}[\rho_0, \lambda, X] = \operatorname{Tr} \left[ \rho L^2 \right] + \lambda(1 - \operatorname{tr}(\rho_0)) + \operatorname{tr}(X\rho_0)
$$
其中 $\lambda$ 是对应迹约束的实数乘子，$X \succeq 0$ 是对应半正定性约束的算符乘子。
KKT条件要求在最优点 $(\rho_0^*, L^*, \lambda^*, X^*)$ 处 （**如何理解额外引入的变量L**？），拉格朗日函数对 $\rho_0$ 的梯度为零，并且满足互补松弛性。这导出了如下方程组：
$$
\left\{\begin{array}{l}
\text{Stationarity:} \quad 2(\partial_\theta \mathcal{E}_\theta)^\dagger(L^*) - \mathcal{E}_\theta^\dagger((L^*)^2) - \lambda^*\mathbb{I} + X^* = 0 \\
\text{Primal Feasibility:} \quad \operatorname{tr}(\rho_0^*) = 1, \quad \rho_0^* \succeq 0 \\
\text{Dual Feasibility:} \quad X^* \succeq 0 \\
\text{Complementary Slackness:} \quad \operatorname{tr}(X^* \rho_0^*) = 0 \quad (\Leftrightarrow X^*\rho_0^*=0)
\end{array}\right.
$$
该方程组给出了使Fisher信息量取得极值的初态 $\rho_{0}^*$ 所需满足的条件。注意 $L^*$ 本身也依赖于 $\rho_0^*$。
将互补松弛性代入驻点条件，我们得到一个关键的本征方程：
$$
\left( 2(\partial_\theta \mathcal{E}_\theta)^\dagger(L^*) - \mathcal{E}_\theta^\dagger((L^*)^2) \right) \rho_0^* = \lambda^* \rho_0^*
$$
这表明，最优初态 $\rho_0^*$ 的支撑空间必须位于算符 $M(L^*) := 2(\partial_\theta \mathcal{E}_\theta)^\dagger(L^*) - \mathcal{E}_\theta^\dagger((L^*)^2)$ 对应于本征值 $\lambda^*$ 的本征子空间内。由于我们是在最大化QFI，$\lambda^*$ 应该是 $M(L^*)$ 的最大本征值。

关键问题是这个推导是否能够帮我找到或者估计scaling？找到最值点之后如何代入QFI中？

上述讨论与计算QFI的**跷跷板迭代 (Iterative See-Saw, ISS)** 方法有关 {（[[0 fleeting/论文阅读笔记/Quantum metrology using quantum combs and tensor network formalism\|kurdzialek2024QuantumMetrologyUsing]] Eq. (10)）}。
- [[单信道的最优初态：跷跷板迭代ISS方法\|单信道的最优初态：跷跷板迭代ISS方法]] 
- [[并联策略最优初态的分析\|并联策略最优初态的分析]] 
## 多参数估计
参考[[liu2020QuantumFisherInformation\|liu2020QuantumFisherInformation]]，[[Advances in multiparameter quantum sensing and metrology\|Advances in multiparameter quantum sensing and metrology]]。
- [[多参数量子精密测量\|多参数量子精密测量]] 
...
#### Fisher信息量的重参数化 reparametrization
...
- 量子纠错直接应用在量子精密测量上显得有些力不从心是因为量子纠错本身的目标不是精密测量，而是为了获得尽可能高的保真度。它有可能会让等效的哈密顿量变得没有意义，投影测量+恢复的策略在提高态的纯度、与原来的态之间的保真度的同时也使得部分信息被丢掉了，丢失信息（灵敏度）是非常直观的一件事情，因为Fisher信息量关于channel是单调递减的，恢复纯度和丢失信息之间是否存在一个平衡？描述、量化这个平衡是非常关键的。控制与动力学的关系如何考虑？先使用无敌模式（或许不是那么无敌），加入噪声，但是又可以加入任意的控制。什么样的R是最好的？
- 权衡：R造成的Fisher信息量下降和纯度的提升（以便提高后续Fisher信息量的积累速率）。
- 打pulse不会导致Fisher信息量的下降，而且又能有机会，看上去非常地有吸引力。
- 缺少具体的模型和例子。这些例子对于建立物理直觉非常重要。
- 有没有可能R做成反馈控制，经典的if-then能够被看做反馈控制吗
- 对于每一个channel来说都存在一个最优的态（如果是拓展信道就是最优的纯态，最优是指如果初始时和参数无关，那么输出的末态是对参数最灵敏的，但如果初始时就和参数有关情况是否会发生一些变化？也就是我是在考虑中途的最优化问题而不是从始到终到最优化问题。）

[[Pasted image 20240222161808.png]]
文献阅读
- [[0 fleeting/论文阅读笔记/Faist-2023-Time-Energy Uncertainty Relation for Noisy Quantum Metrology\|Faist-2023-Time-Energy Uncertainty Relation for Noisy Quantum Metrology]]
- [[0 fleeting/论文阅读笔记/Achieving the Heisenberg limit in quantum metrology using quantum error correction\|Achieving the Heisenberg limit in quantum metrology using quantum error correction]]

## 综述：临界量子计量
  
- ([Garbe et. al., 2019](https://www.paperdigest.org/paper/?paper_id=arxiv-1910.00604)) design different metrological protocols that make use of the superradiant phase transition of the quantum Rabi model, a finite-component system composed of a single two-level atom interacting with a single bosonic mode. ([Garbe et. al., 2019](https://www.paperdigest.org/paper/?paper_id=arxiv-1910.00604)) show that, in spite of the critical slowing down, critical quantum optical systems can lead to a quantum-enhanced time-scaling of the quantum Fisher information, and so of the measurement sensitivity.   ([Gietka et. al., 2021](https://www.paperdigest.org/paper/?paper_id=arxiv-2103.12939)) show that the quantum Fisher information attained in an adiabatic approach to critical [quantum metrology](https://en.wikipedia.org/wiki?curid=3975868) cannot reach the Heisenberg limit of precision and therefore \emph{regular} quantum metrology under optimal settings is always superior. ([Gietka et. al., 2021](https://www.paperdigest.org/paper/?paper_id=arxiv-2103.12939)) argue that even though shortcuts to adiabaticity can arbitrarily decrease the time of preparing critical ground states, they cannot be used to achieve or overcome the Heisenberg limit for quantum parameter estimation in adiabatic critical quantum metrology.   Due to the critical slowing down, the protocol duration time is of utmost relevance in critical quantum metrology. ([Garbe et. al., 2021](https://www.paperdigest.org/paper/?paper_id=doi.org_10.1088_2058-9565_ac6ca5)) provide a comprehensive analysis of the scaling of the quantum Fisher information for different families of protocols that create a continuous connection between static and dynamical approaches.   ([Gietka et. al., 2021](https://www.paperdigest.org/paper/?paper_id=arxiv-2110.04048)) present an improved critical quantum metrology protocol which relies on quenching a system exhibiting a superradiant quantum phase transition beyond its critical point. ([Gietka et. al., 2021](https://www.paperdigest.org/paper/?paper_id=arxiv-2110.04048)) show that this approach can lead to an exponential increase of the quantum Fisher information in time with respect to existing critical quantum metrology protocols relying on quenching close to the critical point and observing power law behaviour.   Due to the critical slowing down, the protocol duration time is of utmost relevance in critical quantum metrology. ([Garbe et. al., 2021](https://www.paperdigest.org/paper/?paper_id=arxiv-2110.04144)) provide a comprehensive analysis of the scaling of the quantum Fisher information for different families of protocols that create a continuous connection between static and dynamical approaches.   ([Gietka, 2021](https://www.paperdigest.org/paper/?paper_id=arxiv-2111.12206)) present an alternative protocol allowing for the preparation of critical states that instead of suffering from the critical slowing-down benefits from the critical speeding-up. ([Gietka, 2021](https://www.paperdigest.org/paper/?paper_id=arxiv-2111.12206)) discuss the application of the adiabatic speed-up protocol in quantum metrology and compare its performance with critical quantum metrology.   The quantum Rabi model (QRM) with linear coupling between light mode and [qubit](https://en.wikipedia.org/wiki?curid=25284) exhibits the analog of a second-order phase transition for vanishing mode frequency which allows for criticality-enhanced quantum metrology in a few-body system. ([Ying et. al., 2022](https://www.paperdigest.org/paper/?paper_id=pubmed-35892995)) show that the QRM including a nonlinear coupling term exhibits much higher measurement precisions due to its first-order-like phase transition at frequency, avoiding the detrimental slowing-down effect close to the critical point of the linear QRM.   ([Salvia et. al., 2022](https://www.paperdigest.org/paper/?paper_id=pubmed-37390423)) investigate critical quantum metrology, that is, the estimation of [parameters](https://en.wikipedia.org/wiki?curid=25065) in many-body systems close to a quantum critical point, through the lens of Bayesian inference theory. ([Salvia et. al., 2022](https://www.paperdigest.org/paper/?paper_id=pubmed-37390423)) first derive a no-go result stating that any nonadaptive strategy will fail to exploit quantum critical enhancement (i.e., precision beyond the shot-noise limit) for a sufficiently large number of [particles](https://en.wikipedia.org/wiki?curid=30778041) N whenever the prior knowledge is limited.   ([Ostermann et. al., 2023](https://www.paperdigest.org/paper/?paper_id=arxiv-2312.04176)) show that the performance of critical quantum metrology protocols, counter-intuitively, can be enhanced by finite temperature. ([Ostermann et. al., 2023](https://www.paperdigest.org/paper/?paper_id=arxiv-2312.04176)) show that the temperature enhancement of the quantum Fisher information can be achieved by adiabatic preparation of the critical state and by preparing it directly in the proximity of the critical point.   Other influential work includes ([Liu et. al., 2021](https://www.paperdigest.org/paper/?paper_id=doi.org_10.1038_s41534-021-00507-x)).
- （Garbe 等人，2019）设计了不同的计量协议，利用量子拉比模型的超辐射相变，这是一个由与单一玻色子模式相互作用的单个两能级原子组成的有限组件系统。 （Garbe 等人，2019）表明，尽管存在临界减速，但临界量子光学系统可以导致量子费希尔信息的量子增强时间缩放，从而提高测量灵敏度。 （Gietka 等人，2021）表明，在临界量子计量学的绝热方法中获得的量子费希尔信息无法达到海森堡的精度极限，因此最佳设置下的 \emph{常规} 量子计量学总是优越的。 （Gietka et. al., 2021）认为，尽管绝热的捷径可以任意减少准备临界基态的时间，但它们不能用于实现或克服绝热临界量子计量中量子参数估计的海森堡极限。由于关键减速，协议持续时间在关键量子计量中至关重要。 （Garbe 等人，2021）对不同系列协议的量子费希尔信息的缩放进行了全面分析，这些协议在静态和动态方法之间建立了连续的联系。 （Gietka 等人，2021）提出了一种改进的临界量子计量协议，该协议依赖于对表现出超出其临界点的超辐射量子相变的系统进行淬火。 （Gietka 等人，2021）表明，相对于依赖于接近临界点的淬火和观察幂律行为的现有关键量子计量协议，这种方法可以导致量子费希尔信息及时呈指数增长。由于关键减速，协议持续时间在关键量子计量中至关重要。 （Garbe 等人，2021）对不同系列协议的量子费希尔信息的缩放进行了全面分析，这些协议在静态和动态方法之间建立了连续的联系。 （Gietka，2021）提出了一种替代协议，允许准备临界状态，而不是遭受临界减速，并受益于临界加速。 （Gietka，2021）讨论绝热加速协议在量子计量中的应用，并将其性能与临界量子计量进行比较。光模式和量子位之间具有线性耦合的量子拉比[[模型\|模型]] (QRM) 表现出消失模式频率的二阶相变模拟，这允许在少体系统中进行临界增强的量子计量。 （Ying 等人，2022）表明，包含非线性耦合项的 QRM 由于其频率处的一阶相变而表现出更高的测量精度，避免了接近临界点的有害减速效应。线性 QRM。 （Salvia 等人，2022）研究临界量子计量学，即通过贝叶斯推理理论的视角估计多体系统中接近量子临界点的参数。 （Salvia 等人，2022）首先得出一个不可行的结果，表明任何非自适应策略都将无法利用量子临界增强（即超出散粒噪声极限的精度）来获得足够多的粒子 N，只要先验知识有限。 （Ostermann 等人，2023）表明，与直觉相反，关键量子计量协议的性能可以通过有限温度来增强。 （Ostermann 等人，2023）表明，量子费希尔信息的温度增强可以通过临界态的绝热制备以及直接在临界点附近制备来实现。其他有影响力的工作包括（Liu et. al., 2021）。

[[Pasted image 20240317150118.png]]
这个公式是错的？Characterization of Unruh channel in the context of open quantum systems

- 指数增长的量子精密测量
	- [[Exponentially Enhanced Quantum Metrology\|Exponentially Enhanced Quantum Metrology]]
	- [[Exponential enhancement of quantum metrology using continuous variables\|Exponential enhancement of quantum metrology using continuous variables]]
	- 