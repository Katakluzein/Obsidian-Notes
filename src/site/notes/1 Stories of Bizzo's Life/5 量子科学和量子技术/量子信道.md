...
$$
\mathcal{N}(\mathbb{1}_A) = \mathbb{1}_B
$$

**证明**：
1.  我们从幺正通道的定义开始： $\mathcal{N}(\mathbb{1}_A) = \mathbb{1}_B$。
2.  对这个等式的两边取迹（trace）：
    $$
    \text{tr}(\mathcal{N}(\mathbb{1}_A)) = \text{tr}(\mathbb{1}_B)
    $$
3.  根据量子通道的定义，它必须是 **保迹的 (trace-preserving)**。这意味着对于任何输入算子 $X$，都有 $\text{tr}(\mathcal{N}(X)) = \text{tr}(X)$。我们将这个性质应用到等式的左边：
    $$
    \text{tr}(\mathcal{N}(\mathbb{1}_A)) = \text{tr}(\mathbb{1}_A)
    $$
4.  将第2步和第3步的结果结合起来，我们得到：
    $$
    \text{tr}(\mathbb{1}_A) = \text{tr}(\mathbb{1}_B)
    $$
5.  将迹用维度替换，我们最终得到：
    $$
    d_A = d_B
...
$$
\mathcal{N}(\mathbb{1}_A) \le \mathbb{1}_B
$$
在算子理论中，不等式 $X \le Y$ 意味着算子 $Y - X$ 是一个正半定算子 (positive semidefinite operator)。所以，这里的定义等价于：
$$
\mathbb{1}_B - \mathcal{N}(\mathbb{1}_A) \ge 0
$$
其中 $\ge 0$ 表示是正半定的。

**证明**：
1.  我们从亚幺正通道的定义开始： $\mathbb{1}_B - \mathcal{N}(\mathbb{1}_A)$ 是一个正半定算子。
2.  正半定算子的一个基本性质是它的迹是非负的。因此，我们对这个算子取迹：
    $$
    \text{tr}(\mathbb{1}_B - \mathcal{N}(\mathbb{1}_A)) \ge 0
    $$
3.  利用迹的线性性质，我们可以将其分解为：
    $$
    \text{tr}(\mathbb{1}_B) - \text{tr}(\mathcal{N}(\mathbb{1}_A)) \ge 0
    $$
4.  同样，由于 $\mathcal{N}$ 是一个保迹的量子通道，我们有 $\text{tr}(\mathcal{N}(\mathbb{1}_A)) = \text{tr}(\mathbb{1}_A)$。将其代入不等式：
    $$
    \text{tr}(\mathbb{1}_B) - \text{tr}(\mathbb{1}_A) \ge 0
    $$
5.  最后，将迹用维度替换，我们得到：
    $$
    d_B - d_A \ge 0
