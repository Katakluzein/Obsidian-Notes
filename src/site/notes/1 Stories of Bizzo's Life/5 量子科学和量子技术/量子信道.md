...
- 参考： [quantumchannelzoo.org](https://www.quantumchannelzoo.org/) 
...
- 带有物理动机的公理(公理化定义)
  ...
  其中，后面两种表示可以视为Stinespring膨胀定理的推论。
...
=&\mathrm{tr}_{A^*}(\mathsf{E}^{BA^*}~\mathbb{I}^B\otimes \rho^{A\top})\\
...
**笔记**
- [[蔡氏同构的逆构成全正映射\|蔡氏同构的逆构成全正映射]] 
	- [[蔡氏同构的逆构成全正映射#蔡氏同构的逆构成全正映射的检验\|蔡氏同构的逆构成全正映射#蔡氏同构的逆构成全正映射的检验]] (这个地方一旦改成就地引用就会导致网页部署失败)
...
\end{aligned}
$$
[[蔡氏矩阵与Kraus算符的关系\|蔡氏矩阵与Kraus算符的关系]] 为
$$
...
$$
下面是另一种约定下的推导
*   信道: $\mathcal{E}(\rho) = \sum_k E_k \rho E_k^\dagger$
*   最大纠缠态: $|\Omega\rangle = \sum_i |i\rangle_A \otimes |i\rangle_{A'}$
*   Choi矩阵 (新约定): $\mathsf{E}' = (\mathcal{I}_A \otimes \mathcal{E}_{A' \to B})(|\Omega\rangle\langle\Omega|)$
*   目标: 将 $\mathsf{E}'$ 表示为 $\sum_k |\psi_k\rangle\langle\psi_k|$ 的形式，并找出 $|\psi_k\rangle$ 与 $E_k$ 的关系。
...
&= (\mathcal{I} \otimes \mathcal{E}) \left[ \left(\sum_i |i\rangle_A|i\rangle_{A'}\right) \left(\sum_j \langle j|_A\langle j|_{A'}\right) \right] & & \text{(1. 展开纠缠态)} \\
&= \sum_{i,j} (\mathcal{I} \otimes \mathcal{E}) \left( |i\rangle_A\langle j|_A \otimes |i\rangle_{A'}\langle j|_{A'} \right) & & \text{(2. 将求和提到前面)} \\
&= \sum_{i,j} \mathcal{I}(|i\rangle_A\langle j|_A) \otimes \mathcal{E}(|i\rangle_{A'}\langle j|_{A'}) & & \text{(3. 超算符作用于张量积)} \\
&= \sum_{i,j} |i\rangle_A\langle j|_A \otimes \left( \sum_k E_k |i\rangle_{A'}\langle j|_{A'} E_k^\dagger \right) & & \text{(4. 代入Kraus算符定义)} \\
&= \sum_k \sum_{i,j} |i\rangle_A\langle j|_A \otimes E_k |i\rangle_{A'}\langle j|_{A'} E_k^\dagger & & \text{(5. 交换求和顺序)} \\
&= \sum_k \left( \sum_i |i\rangle_A \otimes E_k |i\rangle_{A'} \right) \left( \sum_j \langle j|_A \otimes \langle j|_{A'} E_k^\dagger \right) & & \text{(6. 关键一步：重新组合成 Ket-Bra 形式)} \\
&= \sum_k |\psi_k\rangle \langle\psi_k| & & \text{(7. 定义中间向量 } |\psi_k\rangle \text{)}
\end{aligned}
$$

现在，我们来确定 $|\psi_k\rangle$ 的具体形式：
$$
\begin{aligned}
|\psi_k\rangle
&= \sum_i |i\rangle_A \otimes (E_k |i\rangle_{A'}) & & \text{(A. 从第(6)步得到的定义)} \\
&= \sum_i |i\rangle_A \otimes \left( \sum_l |l\rangle_B \langle l|_B E_k |i\rangle_{A'} \right) & & \text{(B. 在B空间插入单位算符)} \\
&= \sum_{i,l} \langle l|_B E_k |i\rangle_{A'} \cdot (|i\rangle_A \otimes |l\rangle_B) & & \text{(C. 提取系数并重排)} \\
&= \sum_{i,l} (E_k)_{li} \cdot (|i\rangle_A \otimes |l\rangle_B) & & \text{(D. 将系数写成矩阵元形式)} \\
&= \sum_{i,l} (E_k^T)_{il} \cdot (|i\rangle_A \otimes |l\rangle_B) & & \text{(E. 利用转置定义 } (M^T)_{ab}=(M)_{ba} \text{)} \\
&= |E_k^T\rangle\rangle & & \text{(F. 根据CJI向量化定义)}
\end{aligned}
$$

### **结论**

将第(F)步的结果代入第(7)步，我们得到：
$$
\mathsf{E}' = \sum_k |E_k^T\rangle\rangle \langle\langle E_k^T|
$$
这证明了，采用 $(\mathcal{I} \otimes \mathcal{E})$ 约定的Choi矩阵，是**转置后的Kraus算符** $\{E_k^T\}$ 的Choi-Jamiołkowski表示的和。
...
#### **命题：Kraus算符和表示的等距自由度 (Isometry Freedom)**

设 $\mathcal{E}: B(\mathsf{H}_1) \to B(\mathsf{H}_2)$ 是一个量子操作，它有两组不同的 Kraus 算符和表示：
1.  $\mathcal{E}(\cdot) = \sum_{i=1}^{n} E_i (\cdot) E_i^\dagger$
2.  $\mathcal{E}(\cdot) = \sum_{j=1}^{m} F_j (\cdot) F_j^\dagger$

那么，这两组 Kraus 算符 $\{E_i\}_{i=1}^n$ 和 $\{F_j\}_{j=1}^m$ 可以通过一个**等距矩阵 (isometry matrix)** $V$ 相互联系。具体来说：

- 如果 $n \ge m$，则存在一个 $n \times m$ 的矩阵 $V$ 满足 $V^\dagger V = \mathbb{I}_m$（即 $V$ 是一个等距矩阵），使得：
  $$
  E_i = \sum_{j=1}^{m} V_{ij} F_j
  $$

- 反之，如果两组 Kraus 算符通过一个等距矩阵（或更一般地，一个满足 $V^\dagger V = \mathbb{I}$ 的矩阵）相联系，那么它们描述的是同一个量子操作。

**证明**

1.  **(等距关系 $\Rightarrow$ 同一操作)**
    假设 $E_i = \sum_{j=1}^{m} V_{ij} F_j$，其中 $V$ 是一个 $n \times m$ 的矩阵，满足 $V^\dagger V = \mathbb{I}_m$。我们来计算由 $\{E_i\}$ 定义的量子操作：
    $$
    \begin{aligned}
    \sum_{i=1}^{n} E_i (\cdot) E_i^\dagger &= \sum_{i=1}^{n} \left( \sum_{j=1}^{m} V_{ij} F_j \right) (\cdot) \left( \sum_{k=1}^{m} V_{ik} F_k \right)^\dagger \\
    &= \sum_{j,k=1}^{m} \left( \sum_{i=1}^{n} V_{ij} \overline{V_{ik}} \right) F_j (\cdot) F_k^\dagger \\
    &= \sum_{j,k=1}^{m} \left( \sum_{i=1}^{n} (V^\dagger)_{ki} V_{ij} \right) F_j (\cdot) F_k^\dagger \\
    &= \sum_{j,k=1}^{m} (V^\dagger V)_{kj} F_j (\cdot) F_k^\dagger
    \end{aligned}
    $$
    由于 $V^\dagger V = \mathbb{I}_m$，即 $(V^\dagger V)_{kj} = \delta_{kj}$，上式变为：
    $$
    \sum_{j=1}^{m} \delta_{jj} F_j (\cdot) F_j^\dagger = \sum_{j=1}^{m} F_j (\cdot) F_j^\dagger
    $$
    这证明了两组 Kraus 算符描述的是同一个量子操作。

2.  **(同一操作 $\Rightarrow$ 等距关系)**
    设两组 Kraus 算符描述同一个操作。不失一般性，我们可以通过添加零算符的方式，使得两组 Kraus 算符的数量相同，即 $n=m$。如果数量不同，例如 $n>m$，我们可以定义 $F_j = 0$ for $j = m+1, \dots, n$。
    -   **构建系数矩阵**: 设 $\mathsf{H}_1$ 和 $\mathsf{H}_2$ 的维度分别为 $d_1, d_2$。我们可以在算符空间 $B(\mathsf{H}_1, \mathsf{H}_2)$ 中选择一组正交归一基 $\{K_k\}_{k=1}^{d_1 d_2}$。然后，将 Kraus 算符在这组基上展开：
        $$
        E_i = \sum_{k=1}^{d_1 d_2} e_{ik} K_k, \quad F_j = \sum_{k=1}^{d_1 d_2} f_{jk} K_k
        $$
        这里，$E = (e_{ik})$ 是一个 $n \times (d_1 d_2)$ 的系数矩阵，$F = (f_{jk})$ 是一个 $m \times (d_1 d_2)$ 的系数矩阵。
    -   **利用 Choi 矩阵**: 一个量子操作 $\mathcal{E}$ 的 Choi 矩阵可以写成 $\mathsf{E} = \sum_k |E_k\rangle\!\rangle\langle\!\langle E_k|$，其中 $|E_k\rangle\!\rangle$ 是 $E_k$ 的向量化表示。由于两组 Kraus 算符描述同一个操作，它们的 Choi 矩阵必然相同：$$
        \sum_{i=1}^{n} |E_i\rangle\!\rangle\langle\!\langle E_i| = \sum_{j=1}^{m} |F_j\rangle\!\rangle\langle\!\langle F_j|
        $$        将算符展开代入，并利用 $|K_k\rangle\!\rangle$ 构成的正交基，可以得到：
        $$
        E^\dagger E = F^\dagger F
        $$
        这里 $E^\dagger$ 是 $E$ 的共轭转置，是一个 $(d_1 d_2) \times n$ 的矩阵。$E^\dagger E$ 和 $F^\dagger F$ 都是 $(d_1 d_2) \times (d_1 d_2)$ 的半正定矩阵。
    -   **应用极分解**: 根据矩阵理论，如果 $E^\dagger E = F^\dagger F$，则存在一个**等距矩阵** $V$ (从 $F$ 的列向量张成的空间的像空间到 $E$ 的列向量张成的空间的像空间)，使得 $E = V F$。
        -   更具体地说，考虑 $E$ 和 $F$ 的**极分解 (polar decomposition)**：$E = U_E |E|$ 和 $F = U_F |F|$，其中 $U_E, U_F$ 是等距矩阵（isometries），$|E| = \sqrt{E^\dagger E}$ 和 $|F| = \sqrt{F^\dagger F}$ 是半正定矩阵。
        -   由 $E^\dagger E = F^\dagger F$ 可知 $|E| = |F|$。
        -   因此，$E = U_E |E| = U_E |F|$，$F = U_F |F|$。我们可以定义 $V = U_E U_F^\dagger$。这个 $V$ 将 $F$ 的像空间映射到 $E$ 的像空间，并且满足 $E = V F$。由于 $U_E, U_F$ 是等距矩阵，$V$ 也是一个等距矩阵。
    -   **得到最终关系**: $E = V F$ 这个矩阵关系可以翻译回 Kraus 算符的关系：$$
        e_{ik} = \sum_{j=1}^m V_{ij} f_{jk}
        $$        将 Kraus 算符的展开式代入，即可得到：$$
        E_i = \sum_{k=1}^{d_1 d_2} \left( \sum_{j=1}^m V_{ij} f_{jk} \right) K_k = \sum_{j=1}^m V_{ij} \left( \sum_{k=1}^{d_1 d_2} f_{jk} K_k \right) = \sum_{j=1}^m V_{ij} F_j
        $$


**笔记**
- [[整理前，量子信道的幺正自由度\|整理前，量子信道的幺正自由度]] 
- 如果设$\mathsf{H}_1$和$\mathsf{H_2}$都是$d$维的，而且设$\{E_i\}$有$n$个Kraus算符，$\{F_i\}$有$m$个Kraus算符，那么$E^\dagger_{d^2\times n} E_{n \times d^2 }=F^\dagger_{d^2\times m} F_{m \times d^2 }$ ，这两组系数可以用一个isometry联系起来，$V$是$n\times m$矩阵满足$E=V F$。
...
- 实际上$V$有一个显式解 (lei2024DilationDiscriminationUhlmanns)，==但是这里面的公式好像不太对？还是我理解错了？==。
- isometry $W$有一个显式解，$W=\sum_i |i\rangle_A \otimes K_i$ ([[A theoretical framework for quantum networks\|A theoretical framework for quantum networks]] Th.3)
- 如果没有直积态的条件就不能写成Kraus算符和的形式、也不能被视为量子信道。
- 从原信道到对偶信道的系统环境耦合表示 (serafini2017QuantumContinuousVariables, (5.61))：
  $$
  ...
  \end{aligned}
  $$
  上式中的$\hat{A}$是任意的，因此有[[对偶信道\|对偶信道]]的系统环境耦合表示
  $$
...
- [2102.02054](https://arxiv.org/pdf/2102.02054) 这篇文章有很多信道的例子。
