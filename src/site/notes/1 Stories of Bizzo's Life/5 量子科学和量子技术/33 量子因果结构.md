...
参考[[ball2017HowQuantumTrickery\|ball2017HowQuantumTrickery]]。在日常生活的经验中，任何事件的发生都有确定的因果顺序。即便在正统的量子力学中，我们也完整地继承了牛顿时空观，例如在纠缠粒子对的测量实验中，我们仍然假设$B$观察到的效应发生在$A$进行测量之后，$A$不可能对她测量之前的物理状态产生影响。然而人们发现量子力学可能比原来所认识到的更加奇怪，两个事件的发生可能可以没有确定的因果关系甚至没有确定的时间顺序，量子信道的不同发生顺序、演化方向可以被相干地叠加，分别对应量子开关和量子时间翻转[[stromberg2024ExperimentalSuperpositionQuantum\|stromberg2024ExperimentalSuperpositionQuantum]]，这两个违反因果顺序的现象都已经在实验上被实现了。在具有不定因果的实验情景下谈论哪件事情是原因、或者哪件事情发生在前是没有意义的。

因果关系本质上就是关于事物在时间和空间之间相互影响关系，所以新的因果理论有助于量子力学和相对论的融合。我们需要更加深入地思考和研究量子因果结构，构建更加完善和激进的因果理论来解释这些奇怪的现象。与此同时，操纵不定因果将帮助我们在加速量子计算、提高精密测量精度方面更进一步，不定因果关系在量子通信、反转量子动力学、量子热力学方面都有应用（前面所有这些应用，参考[[antesberger2024HigherOrderProcessMatrix\|antesberger2024HigherOrderProcessMatrix]]的引言部分）。
...
根据[[0 fleeting/论文阅读笔记/Simulating Noncausality with Quantum Control of Causal Orders\|Simulating Noncausality with Quantum Control of Causal Orders]]，这些奇异的非因果结构在物理层面上究竟如何表现——它们是否能够在实际中被实现，以及在什么样的物理环境中才会出现——这些仍然是一个未解之谜。一方面，卢加诺过程具备可纯化性（即可以以单位算子的形式进行扩展），这一特性被视为判断其是否具有物理可实现性的重要标准[20]；此外，这种过程可以通过时间离散化的子系统[21, 22]、依赖于观察者的局部事件[23]，或者通过特定的量子电路来实现[24]。另一方面，细致的分析表明，在满足相对论因果律的固定时空中，因果不等式的违反是不可能发生的[25, 26]；这类现象实际上依赖于与闭合类时曲线的相互作用。此外，目前已知的最大一类在物理上可实现但不存在明确因果顺序的过程，就是那些能够对因果顺序进行量子控制的量子电路 [[0 fleeting/论文阅读笔记/Quantum Circuits with Classical Versus Quantum Control of Causal Order\|Quantum Circuits with Classical Versus Quantum Control of Causal Order]] ，其中包括著名的“量子开关”[28]——这类过程所产生的仅仅只是因果相关性[27, 29–31]。卢加诺过程究竟是会在广义相对论中出现，还是会在量子理论与引力的交叉领域产生[32, 33]，又或者如何实现或观测到这种过程的局部表现形式，这些目前都还无从知晓。

经过对不定因果过程的不断研究，产生了许多有用的物理概念，帮我们更精细地对不定因果现象进行分类和刻画

- 固定因果顺序的量子过程。量子梳。各个观测者有预先确定的全序，时间轴上靠后的人不能向靠前的人通信。
-  因果可分过程（[[因果不可分性\|因果不可分性]]）。对两体过程来说，因果可分过程就是两种可能的顺序过程矩阵的经典线性叠加(可能超过两项)，已经证明可幺正拓展的两体过程都是因果可分的[[barrett2021CyclicQuantumCausal\|barrett2021CyclicQuantumCausal]]。对于多体过程，参见[[On the definition and characterisation of multipartite causal (non) separability\|On the definition and characterisation of multipartite causal (non) separability]]。
-  因果不可分过程。不是因果可分的过程。例如(三体（[[ognyanoreshkov2019TimedelocalizedQuantumSubsystems\|ognyanoreshkov2019TimedelocalizedQuantumSubsystems]] (10)）或四体版本)量子开关（量子开关可以被视为四节点幺正过程，因此它作为两节点过程是可幺正拓展的，进而是因果可分的，这与三体或者四题量子开关不可分之间并不矛盾）。
-  非因果过程（[[因果关联\|因果过程]]）。根据预选择的过程理论，满足因果性的概率分布需要满足特定的等式，这些等式划定了一个因果凸包，凸包的面能帮我们制作一些因果不等式，违反因果不等式的过程被称为非因果过程(acausal, non causal)。当前的对于不定因果过程能否实现的争议主要集中在那些非因果的过程，有人提出了用时间非局域的过程来在实验上“实现”那些非因果的过程，已经证明存在三体非因果过程，能够用时间非局域子系统的方式来实现[[wechs2023ExistenceProcessesViolating\|wechs2023ExistenceProcessesViolating]]。**时间非局域子系统**(delocalised)：对于多个不同时刻的Hilbert空间的张量积$\mathsf{H}\equiv \bigotimes \mathsf{H}_{t_i}$，对$\mathsf{H}$选取新的张量积结构，如果新的张量积因子(子系统)没有确定的时间，则称其为时间非局域子系统。
- 可幺正拓展的过程，可以通过加入辅助系统，使得过程矩阵拓展为幺正信道的过程。这些过程被认为是能够在实验上实现的必要条件，因为它保持了量子操作的可逆性，也就是说，当你在每个插槽插入幺正信道的时候，可幺正拓展的过程会返回给你一个幺正信道[[wechs2023ExistenceProcessesViolating\|wechs2023ExistenceProcessesViolating]]。

![2025-05-15-16-16-29.png](/img/user/2025-05-15-16-16-29.png)
...
- [[不定因果中的开放问题\|不定因果中的开放问题]] 
## 高阶量子映射与过程矩阵
问题：与量子张量网络的关系是什么？张量网络通常是指量子梳，具有确定因果顺序的(一种没有完全确定因果结构的约束，可以用投影算符刻画)，而过程矩阵通常不需要具有确定的因果结构。[[链积\|链积]]用于缩并过程矩阵。
### 定义与等价刻画
...
> [!definition] 概率归一化的过程矩阵(算符)
> \cite[Def.3.7 - lorenz2020QuantumCausalStructure]
> \label{def:processmatrix}
...
> - \label{def:nosignalling} 对于两节点过程$\sigma_{AB}$（满足ICO合法性的前提下），下列等价{（[[barrett2021CyclicQuantumCausal\|barrett2021CyclicQuantumCausal]] p.3）}
>
...
在$n$体过程矩阵中，在站点之间施加额外的因果假设——特别是无信号假设——可以通过向过程矩阵添加线性约束来实现。例如，对双部过程（bipartite process）的站点 A、B 施加无信号约束等价于 [45]： 
$$W = (\operatorname{Tr}_{A_O B_O} W) \otimes \mathbb{1}^{A_O B_O}, \qquad (9)$$类似地，从 **B 到 A 的单向无信号**对应以下线性约束：  （在此类表达式中，张量因子的顺序已隐含调整，这是初学者通常会碰到的困惑：为什么不同的蔡氏矩阵的约定不会导致不同的过程矩阵表达式？）
...
## 因果关联与因果不等式
- 违反因果不等式的过程 A. Baumeler, A. Feix, and S. Wolf, Phys. Rev. A 90, 042106 (2014).
- 如果给定一个概率分布（产生因果关联、或者产生非因果关联），是否一定存在特定的过程矩阵来实现这种过程？具体应该如何构造？
- 仍然不知道[[Witnessing causal nonseparability\|Witnessing causal nonseparability]]中所提到的最一般的因果关联是如何定义的？
## 量子开关
参见[[量子开关\|量子开关]]
## Lugano(AF-BW)过程
- [[Lugano过程\|Lugano过程]] 
...
- [[项目：不定因果×单参数含时哈密顿量量子精密测量\|项目：不定因果×单参数含时哈密顿量量子精密测量]]
