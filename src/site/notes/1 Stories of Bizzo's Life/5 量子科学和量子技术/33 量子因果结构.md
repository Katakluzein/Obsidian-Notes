...
\begin{itemize}
    \item 固定因果顺序的过程。量子梳。各个观测者有预先确定的全序，靠后的人不能向靠前的人通信。
...
>
$$
\mathcal{D}_A\left(\rho\right)\equiv {}_A(\rho)\equiv \frac{\mathbb{1}_A}{d_A} \operatorname{tr}\left(\rho\right)
$$
...
$$
\sigma _{A_1A_2}
=\mathbb{1}_{A_2^{\mathrm{out}*}}\otimes \mathsf{C}\otimes \rho_{A_1^\mathrm{in}}
=\mathbb{1}_{A_2^{\mathrm{out}*}}\otimes \rho^\mathcal{C}_{A_2^\mathrm{in}|A_1^\mathrm{out}}\otimes \rho_{A_1^\mathrm{in}}
...
$$
\sigma _{A_1A_2}
=\rho_{A_1^\mathrm{in}}\otimes  \mathsf{C}\otimes\mathbb{1}_{A_2^{\mathrm{out}*}}
=\rho_{A_1^\mathrm{in}}\otimes  \rho^\mathcal{C}_{A_2^\mathrm{in}|A_1^\mathrm{out}}\otimes\mathbb{1}_{A_2^{\mathrm{out}*}}
...
> \begin{aligned}
> \rho^\mathcal{L}
> &= (\mathcal{L}\otimes \mathbb{1})\!\left(|\mathbb{1}\rangle\langle\mathbb{1}|\right) \\
...
> &= \mathrm{tr}_{A^{\mathrm{i*}}B^{\mathrm{i*}}}\!\left(|ij\rangle\langle pq|\,\rho^{AB\top}\right)\,|ij\rangle\langle pq|_{A^\mathrm{i*}B^\mathrm{i*}}\otimes \mathbb{1}_{A^\mathrm{o*}B^\mathrm{o*}} \\
> &= \rho^{AB}\otimes \mathbb{1} \;=\; \sigma_{AB}.
> \end{aligned}
> $$
>
...
利用过程矩阵的关于偏迹的性质，可以将过程矩阵本身视为某个信道的蔡氏矩阵。在式\eqref{eq:normalofprocessmatrix}中取$\tau_{A}=\tau_{A^{\mathrm{out}*}}\otimes \mathbb{1}_{A^\mathrm{in}}\in B\left( {A^{\mathrm{out}*}}\otimes  {A^\mathrm{in}}\right)$，则式\eqref{eq:normalofprocessmatrix}成为
$$
...
$$
\mathrm{tr}_{\{{A_i^\mathrm{in}\}}}(\sigma_{A_1\ldots A_n})= \mathbb{1}_{A_1^{\mathrm{out}*}}\otimes \cdots \otimes \mathbb{1}_{A_n^{\mathrm{out}*}}
$$
...
\mathrm{tr}_F\sigma =\sigma_{(n - 1)}\otimes \mathbb{1}_{A_{n- 1}^\mathrm{o}}\\
\mathrm{tr}_{A^\mathrm{i}_{l}}\sigma_{(l )} =\sigma_{(l - 1)}\otimes \mathbb{1}_{A_{l - 1 }^\mathrm{o}} ,~ l= 2,\ldots, n-1 \\
\mathrm{tr}_{A^\mathrm{i}_{1}}\sigma_{(1)} = \mathbb{1}_{P^\mathrm{o}}
\end{gathered}
$$
>
>   eq-eq-defofcomb_2
...
> - $\sigma_{A B}=\sigma_{A B^{\text {in}}} \otimes \mathbb{1}_{B^{\text{out*}}}$
>   此时称**$B$不能向$A$送信**，记作$B\npreceq A$。
...
> [!proof]
> - $(\impliedby)$利用$\mathrm{tr}_{B^\mathrm{out*}}\tau_B=\mathbb{1}_{B^\mathrm{in}}$可得。
>
>   $(\implies)$根据两节点过程矩阵的刻画{（[[oreshkov2012QuantumCorrelationsNo\|oreshkov2012QuantumCorrelationsNo]] Sup.）}，
>
$$
\begin{aligned}
W^{A_1 A_2 B_1 B_2} & =\frac{1}{d_{A_1} d_{B_1}}\left(\mathbb{1}+\sigma^{B \preceq  A}+\sigma^{A \preceq  B}+\sigma^{A \npreceq \nsucceq    B}\right), \\
...
$$
\operatorname{tr}_C\left[\rho_{C D \mid A B}^{\mathcal{U}}\right]=\rho_{D \mid B}^{\mathcal{M}} \otimes \mathbb{1}_{A^*}
$$
>   或等价地
>
$$
...
>   $B^\mathrm{out*}\nrightarrow  A^\mathrm{in}$等价于$\mathrm{tr}_{B^\mathrm{in*}}\left(\sigma_{AB}\right)=\rho^\mathcal{M}_{A^\mathrm{in}|A^\mathrm{out}}\otimes \mathbb{1}_{B^\mathrm{out*}}$。而$B\npreceq A$定义为$\sigma_{AB}=\sigma_{AB^{\mathrm{in}}}\otimes \mathbb{1}_{B^\mathrm{out*}}$。两个定义是等价的，先证$\implies$，根据式\eqref{eq:chaoftwopointporcess}可知，$\sigma^{B\preceq A}=0$，根据定义\ref{def:nosignalling}，这就是$B \npreceq A$；再证$\impliedby$，只要求迹再证明系统$\mathrm{tr}_{B^\mathrm{in}}(\sigma_{AB^\mathrm{in}})$是合法信道即可，正定性显然，只要检验保迹性，实际上利用式\eqref{eq:chaoftwopointporcess}，有$\mathrm{tr}_{A^{\mathrm{in}}B^\mathrm{in}}(\sigma_{AB^\mathrm{in}})=\mathrm{tr}_{A^\mathrm{out*}B}(\sigma_{AB})/d_{B^\mathrm{out*}}=\mathbb{1}_{A^\mathrm{out*}}$
...
>
$$
\rho_{A_i \mid P a\left(A_i\right)} \neq \frac{1}{d_j} \operatorname{tr}_{A_j^{\text{out*}}}\left(\rho_{A_i \mid P a\left(A_i\right)}\right) \otimes \mathbb{1}_{A_j^{\text{out*}}},~\forall ~j
$$
>
>   等价地简记为
>
...
$$
\begin{aligned}
\sigma_{AB}&=\rho_{A^\mathrm{in}}\otimes \rho_{B^\mathrm{in}F|A^\mathrm{out}A^\prime}^\mathrm{CNOT}\otimes \mathbb{1}_{B^{\mathrm{out}*}}\\
& =\rho_{A^\mathrm{in}}\otimes \mathrm{tr}_{A^{\prime*} F}\left(\rho^{\mathrm{CNOT}_{A^\mathrm{out}\rightarrow A^\prime}}_{B^\mathrm{in}F|A^\mathrm{out}A^{\prime}}\rho_{A^\prime}^\top \right)\otimes \mathbb{1}_{B^{\mathrm{out}*}}\\
& =\rho_{A^\mathrm{in}}\otimes \mathrm{tr}_{A^\prime F}\left(\sum |i_1i_2\rangle_{A^{\mathrm{out}*}A^{\prime*}}\langle k_1k_2|\otimes |i_1,i_1 +i_2\rangle_{FB^\mathrm{in}}\langle k_1,k_1 + k_2 |\rho_{A^\prime}^\top\right)\otimes \mathbb{1}_{B^\mathrm{in}}\otimes \mathbb{1}_{B^{\mathrm{out}*}}\\
& =\rho_{A^\mathrm{in}}\otimes \mathrm{tr}_{A^\prime}\left(\sum |k_1i_2\rangle_{A^{\mathrm{out}*}A^{\prime*}}\langle k_1k_2|\otimes|k_1 +i_2\rangle\langle k_1 + k_2| \rho_{A^\prime}^\top\right)\otimes \mathbb{1}_{B^\mathrm{in}}\otimes \mathbb{1}_{B^{\mathrm{out}*}}
\end{aligned}
$$
> [!note]
> - 根据结果可以做出以下分析。显然$B$不能向$A$发信。当$\rho_{A^\prime}$是最大混态的时候，$\sigma_{AB}=\rho_\mathrm{in}\otimes \mathbb{1}_{A^{\mathrm{out}*}B^\mathrm{in}B^{\mathrm{out}*}}$中出现因子$\mathbb{1}_{A^{\mathrm{out}*}}$，$A$不能影响$B$($A$不能向$B$发信)，这是显然的，因为无论$A^\mathrm{out}$输出什么比特，控制非门在输入最大混态的情况下都会输出最大混态，信息被随机性掩盖了。当$\rho_{A^\prime}=|0\rangle\langle 0|$时$A$可以向$B$发信。
...
$$
\sum_{k_1} \operatorname{Tr}_{A_{k_1}^I \alpha_1}\left|w_{\left(\emptyset, k_1\right)}\right\rangle\left\langle w_{\left(\emptyset, k_1\right)}\right|= \mathbb{1}^P
$$
$$
\begin{aligned}
\forall n & =1, \ldots, N-1, \forall \mathcal{K}_n, \\
&   \sum_{k_{n+1} \notin \mathcal{K}_n} \operatorname{Tr}_{A_{k_{n+1}}^I \alpha_{n+1}} \mid w_{ (\mathcal{K}_n, k_{n+1} )} \rangle \langle w_{ (\mathcal{K}_n, k_{n+1} )} | =  \sum_{k_n \in \mathcal{K}_n} \operatorname{Tr}_{\alpha_n} |w_{ (\mathcal{K}_n \backslash k_n, k_n )} \rangle \langle w_{ (\mathcal{K}_n \backslash k_n, k_n )} | \otimes  \mathbb{1}^{A_{k_n}^O},
\end{aligned}
$$
eq-eq-qcqctpcond
$$
\begin{aligned}
& \operatorname{Tr}_{F \alpha_F} |w_{(\mathcal{N}, F)} \rangle \langle w_{(\mathcal{N}, F)} |    \quad=\sum_{k_N \in \mathcal{N}} \operatorname{Tr}_{\alpha_N} \mid w_{ (\mathcal{N} \backslash k_N, k_N )} \rangle \langle w_{ (\mathcal{N} \backslash k_N, k_N )} | \otimes  \mathbb{1}^{A_{k_N}^O}
\end{aligned}
$$
归一化条件中出现的向量的定义为
$$
...
$$
\begin{aligned}
& \operatorname{Tr}_F W=\sum_{k_N \in \mathcal{N}} W_{\left(\mathcal{N} \backslash\left\{k_N\right\}, k_N\right)} \otimes \mathbb{1}^{A_{k_N}^O}, \\ & \forall n=1, \ldots, N-1, \forall \emptyset \subsetneq {\mathcal { K } _ { n } \subsetneq \mathcal { N } ,}   \sum_{k_{n+1} \in \mathcal{N} \backslash \mathcal{K}_n} \operatorname{Tr}_{A_{k_{n+1}}^I} W_{\left(\mathcal{K}_n, k_{n+1}\right)}  =\sum_{k_n \in \mathcal{K}_n} W_{\left(\mathcal{K}_n \backslash\left\{k_n\right\}, k_n\right)} \otimes \mathbb{1}_{k_n}, \\
...
$$\left|V_{\emptyset, \emptyset}^{\rightarrow k_1}\right\rangle_{\mathsf{Seq}} = |\mathbb{1}\rangle\rangle^{P_t A_{k_1}^I} \otimes \sum_{\substack{\pi \in \Pi_{\mathcal{N}}: \\ \pi(1)=k_1}}\langle \mathrm{id}|\pi\rangle^{P_c}  \otimes|\pi\rangle^{\alpha_1} = |\mathbb{1}\rangle\rangle^{P_t A_{k_1}^I} \otimes |\mathrm{id}\rangle^{\alpha_1}\delta_{k_1,1}

$$

后续操作保持不变，就能够得到一个串联过程。如果将后续操作也化简，将控制系统$C_n$全部视为1维的，则有

$$ |V_{\mathcal{K}_{n-1},k_n^{\to k_{n+1}}}\rangle_{\mathsf{Seq}}=|\mathbb{1}\rangle _{A_n^O A_{n + 1}^I}\delta_{k_n,n}\delta_{k_{n+1},n+1}\delta_{\mathcal{K}_{n-1},\{1,\cdots,n\}} $$

$$ |\tilde{V}_{\mathcal{\mathcal { N }} \backslash k_N, k_N}^{ \rightarrow F} \rangle_{\mathsf{Seq}}=|\mathbb{1}^{A_{k_N}^OF}\rangle\delta_{k_N,N} $$

每步的等距操作为

$$    \begin{aligned}
   |\tilde{V}_1  \rangle = & |\mathbb{1}\rangle^{PA_1^I} \otimes    |\emptyset, 1 \rangle^{C_1} \in \mathsf{H}^{P \tilde{A}_1^I \alpha_1 C_1}, \\

   |\tilde{V}_{n+1}  \rangle= &  |\mathbb{1} \rangle^{ A_n^O A_{n + 1}^I} \otimes |\{1,\cdots,n - 1\}, n \rangle^{C_n^{\prime}}  \otimes|\{1,\cdots,n \} , {n+1} \rangle^{C_{n+1}}  \in \mathsf{H}^{\tilde{A}_n^o \alpha_n C_n^{\prime} \tilde{A}_{n+1}^I \alpha_{n+1} C_{n+1}}, \\

   |\tilde{V}_{N+1}  \rangle= &  |\mathbb{1} \rangle^{ A_{N^O}F} \otimes   | \{1,\cdots,N - 1\}, N \rangle^{C_N^{\prime}}  \in \tilde{\mathsf{H}}^{\tilde{A}_N^O \alpha_N C_N^{\prime} F \alpha_F}  
\end{aligned}$$
...
$$
W_{\pi=(1,2,3)} = ||\mathbb{1}\rangle\rangle\langle\langle\mathbb{1}||^{P A_1^I} \otimes ||\mathbb{1}\rangle\rangle\langle\langle\mathbb{1}||^{A_1^O A_2^I} \otimes ||\mathbb{1}\rangle\rangle\langle\langle\mathbb{1}||^{A_2^O A_3^I} \otimes ||\mathbb{1}\rangle\rangle\langle\langle\mathbb{1}||^{A_3^O F}
$$
其中，$||\mathbb{1}\rangle\rangle\langle\langle\mathbb{1}||^{XY}$ 是从希尔伯特空间 $\mathsf{H}^X$ 到 $\mathsf{H}^Y$ 的恒等信道的 Choi 矩阵。为满足命题中的归一化条件，我们假设“过去”空间 $\mathsf{H}^P$ 是一维的，即 $d_P=1$。
...
*   **对于 n=1:** ($\mathcal{K}_0 = \emptyset$)
    $$
    \begin{aligned}
    W_{(\emptyset, 1)} & := ||\mathbb{1}\rangle\rangle\langle\langle\mathbb{1}||^{P A_1^I} \\
    W_{(\emptyset, 2)} & := 0 \\
    W_{(\emptyset, 3)} & := 0
    \end{aligned}
    $$
*   **对于 n=2:** ($\mathcal{K}_1 \subset \mathcal{N}$)
    $$
    \begin{aligned}
    W_{(\{1\}, 2)} & := ||\mathbb{1}\rangle\rangle\langle\langle\mathbb{1}||^{P A_1^I} \otimes ||\mathbb{1}\rangle\rangle\langle\langle\mathbb{1}||^{A_1^O A_2^I} \\
...
*   **对于 n=3:** ($\mathcal{K}_2 \subset \mathcal{N}$)
    $$
    \begin{aligned}
    W_{(\{1,2\}, 3)} & := ||\mathbb{1}\rangle\rangle\langle\langle\mathbb{1}||^{P A_1^I} \otimes ||\mathbb{1}\rangle\rangle\langle\langle\mathbb{1}||^{A_1^O A_2^I} \otimes ||\mathbb{1}\rangle\rangle\langle\langle\mathbb{1}||^{A_2^O A_3^I} \\
...
1.  **正定性条件 ($W_{(\mathcal{K}_{n-1}, k_n)} \geq 0$)**
    我们构造的非零矩阵是 $||\mathbb{1}\rangle\rangle\langle\langle\mathbb{1}||$ 算子（这是一个秩为1的投影算子）的张量积，因此它们是正定的。零矩阵也是半正定的。此条件满足。
...
$$    \begin{aligned}
    ...
    &= \operatorname{Tr}_{P A_1^I}(||\mathbb{1}\rangle\rangle\langle\langle\mathbb{1}||^{P A_1^I}) + 0 + 0 \\
    &= d_P = 1 \quad (\text{因为我们假设 } d_P=1)
    \end{aligned}$$
    此条件满足。

3.  **顶层条件 ($\operatorname{Tr}_F W=\sum_{k_N \in \mathcal{N}} W_{\left(\mathcal{N} \backslash\left\{k_N\right\}, k_N\right)} \otimes \mathbb{1}^{A_{k_N}^O}$)**
    对于 $N=3$，我们需要验证 $\operatorname{Tr}_F W = \sum_{k_3 \in \{1,2,3\}} W_{(\{1,2,3\}\backslash\{k_3\}, k_3)} \otimes \mathbb{1}^{A_{k_3}^O}$。
    *   **等式左边 (LHS):**
$$        \begin{aligned}
        \operatorname{Tr}_F(W_{\pi=(1,2,3)}) &= \operatorname{Tr}_F\left(||\mathbb{1}\rangle\rangle\langle\langle\mathbb{1}||^{P A_1^I} \otimes \dots \otimes ||\mathbb{1}\rangle\rangle\langle\langle\mathbb{1}||^{A_3^O F}\right) \\
        &= ||\mathbb{1}\rangle\rangle\langle\langle\mathbb{1}||^{P A_1^I} \otimes ||\mathbb{1}\rangle\rangle\langle\langle\mathbb{1}||^{A_1^O A_2^I} \otimes ||\mathbb{1}\rangle\rangle\langle\langle\mathbb{1}||^{A_2^O A_3^I} \otimes \mathbb{1}^{A_3^O}
        \end{aligned}$$
    *   **等式右边 (RHS):**
$$        \begin{aligned}
        \text{RHS} &= W_{(\{1,2\},3)} \otimes \mathbb{1}^{A_3^O} + W_{(\{1,3\},2)} \otimes \mathbb{1}^{A_2^O} + W_{(\{2,3\},1)} \otimes \mathbb{1}^{A_1^O} \\
        &= W_{(\{1,2\},3)} \otimes \mathbb{1}^{A_3^O} + 0 + 0 \\
        &= \left(||\mathbb{1}\rangle\rangle\langle\langle\mathbb{1}||^{P A_1^I} \otimes ||\mathbb{1}\rangle\rangle\langle\langle\mathbb{1}||^{A_1^O A_2^I} \otimes ||\mathbb{1}\rangle\rangle\langle\langle\mathbb{1}||^{A_2^O A_3^I}\right) \otimes \mathbb{1}^{A_3^O}
        \end{aligned}$$
    LHS = RHS。此条件满足。

4.  **递归条件 ($\forall n=1, \ldots, N-1, \dots$)**
    *   **验证 n=1:** 我们需要验证 $\sum_{k_2 \in \mathcal{N}\backslash\mathcal{K}_1} \operatorname{Tr}_{A_{k_2}^I} W_{(\mathcal{K}_1, k_2)} = \sum_{k_1 \in \mathcal{K}_1} W_{(\mathcal{K}_1\backslash\{k_1\}, k_1)} \otimes \mathbb{1}^{A_{k_1}^O}$。
        *   **情况 1: $\mathcal{K}_1=\{1\}$**
            *   LHS: $\sum_{k_2 \in \{2,3\}} \operatorname{Tr}_{A_{k_2}^I} W_{(\{1\}, k_2)} = \operatorname{Tr}_{A_2^I} W_{(\{1\}, 2)} + \operatorname{Tr}_{A_3^I} W_{(\{1\}, 3)} = \operatorname{Tr}_{A_2^I} W_{(\{1\}, 2)} + 0 = ||\mathbb{1}\rangle\rangle\langle\langle\mathbb{1}||^{P A_1^I} \otimes \mathbb{1}^{A_1^O}$.
            *   RHS: $\sum_{k_1 \in \{1\}} W_{(\emptyset, k_1)} \otimes \mathbb{1}^{A_{k_1}^O} = W_{(\emptyset, 1)} \otimes \mathbb{1}^{A_1^O} = ||\mathbb{1}\rangle\rangle\langle\langle\mathbb{1}||^{P A_1^I} \otimes \mathbb{1}^{A_1^O}$.
            *   LHS = RHS。
        *   **情况 2: $\mathcal{K}_1=\{2\}$ 或 $\mathcal{K}_1=\{3\}$**
            ...
            *   LHS = RHS。
    *   **验证 n=2:** 我们需要验证 $\sum_{k_3 \in \mathcal{N}\backslash\mathcal{K}_2} \operatorname{Tr}_{A_{k_3}^I} W_{(\mathcal{K}_2, k_3)} = \sum_{k_2 \in \mathcal{K}_2} W_{(\mathcal{K}_2\backslash\{k_2\}, k_2)} \otimes \mathbb{1}^{A_{k_2}^O}$。
        *   **情况 1: $\mathcal{K}_2=\{1,2\}$**
            *   LHS: $\sum_{k_3 \in \{3\}} \operatorname{Tr}_{A_{k_3}^I} W_{(\{1,2\}, k_3)} = \operatorname{Tr}_{A_3^I} W_{(\{1,2\}, 3)} = ||\mathbb{1}\rangle\rangle\langle\langle\mathbb{1}||^{P A_1^I} \otimes ||\mathbb{1}\rangle\rangle\langle\langle\mathbb{1}||^{A_1^O A_2^I} \otimes \mathbb{1}^{A_2^O}$.
            *   RHS: $\sum_{k_2 \in \{1,2\}} W_{(\{1,2\}\backslash\{k_2\}, k_2)} \otimes \mathbb{1}^{A_{k_2}^O} = W_{(\{2\}, 1)} \otimes \mathbb{1}^{A_1^O} + W_{(\{1\}, 2)} \otimes \mathbb{1}^{A_2^O} = 0 + W_{(\{1\}, 2)} \otimes \mathbb{1}^{A_2^O} = ||\mathbb{1}\rangle\rangle\langle\langle\mathbb{1}||^{P A_1^I} \otimes ||\mathbb{1}\rangle\rangle\langle\langle\mathbb{1}||^{A_1^O A_2^I} \otimes \mathbb{1}^{A_2^O}$.
            *   LHS = RHS。
...
$$
\begin{aligned}
\left.\left|V_{\emptyset, \emptyset}^{\rightarrow k_1}\right\rangle\right\rangle & =| \mathbb{1}\rangle\rangle^{P_t A_{k_1}^I} \otimes\left|k_1\right\rangle^{P_c}, \\
\left.\left|V_{\emptyset, k_1}^{\rightarrow k_2}\right\rangle\right\rangle & =| \mathbb{1}\rangle\rangle^{A_{k_1}^O A_{k_2}^I}, \\
\left.\left|V_{\left\{k_1\right\}, k_2}^F\right\rangle\right\rangle & =| \mathbb{1}\rangle\rangle^{A_{k_2}^O F_t} \otimes\left|k_1\right\rangle^{F_c} .
\end{aligned}
$$
...
$$
\begin{aligned}
|w_{\mathsf{DS}} \rangle=        |1\rangle^{P_c}| \mathbb{1} \rangle^{P_t A_1^I}| \mathbb{1} \rangle^{A_1^O A_2^I}| \mathbb{1} \rangle^{A_2^O F_t}|1\rangle^{F_c}       +|2\rangle^{P_c}| \mathbb{1} \rangle^{P_t A_2^I}| \mathbb{1} \rangle^{A_2^O A_1^I}| \mathbb{1} \rangle^{A_1^O F_t}|2\rangle^{F_c}  \in \mathcal{H}^{P_c P_t A_1^{I O} A_2^{I O} F_t F_c},
\end{aligned}
$$
### 例：一个只有部分记忆的量子开关
...
$$
\begin{aligned}
& \left|V^{\rightarrow k_1}\right\rangle=|\mathbb{1}\rangle^{P A_{k_1}^I}\left|k_1\right\rangle^{P_c} \\
& \left|V^{\rightarrow k_2}\right\rangle=\frac{1}{\sqrt{2}}|\mathbb{1}\rangle^{A_{k_1}^O A_{k_3}^I} \\
& \left|V^{\rightarrow k_3}\right\rangle=|\mathbb{1}\rangle^{A_{k_1}^O A_{k_3}^I}\left|k_2\right\rangle^{\alpha_3} \\
& \left|V^{\rightarrow F}\right\rangle=|\mathbb{1}\rangle^{A_{k_3}^O F}\left|k_3\right\rangle^{F_c}|\mathbb{1}\rangle^{\alpha_3 \alpha_F}
\end{aligned}
$$
>
>   最终的蔡氏向量为
>
$$
\left|w_{(\mathcal{N}, F)}\right\rangle=\sum_{\left(k_1, k_2, k_3\right)} \frac{1}{\sqrt{2}}\left|\mathbb{1}^3\right\rangle^{P A_1 A_2 A_3 F}\left|k_1\right\rangle^{P_c}\left|k_2\right\rangle^{\alpha_F}\left|k_3\right\rangle^{F_c}
$$
>
...
\end{array},  \right. \\
& V_{ \{k_1, k_2 \}, k_3}^{\rightarrow F}=\mathbb{1}^{A_{k_3}^O \alpha_3  \rightarrow \alpha_F^{(1)}} \otimes |k_3 \rangle^{\alpha_F^{(2)}},
\end{aligned}
$$
>
...
$$
\begin{aligned}
& V_{\emptyset, \emptyset}^{ \rightarrow k_1}=\frac{1}{\sqrt{3}}\mathbb{1}^{P\rightarrow A_{k_1}^I}, \\
& V_{\emptyset, k_1}^{ \rightarrow k_2}= \left\{\begin{array}{lll}
|0\rangle^{A_{k_2}^I} \langle   0 |^{A_{k_1}^O}   & \text { if } k_2=k_1+1 & (\bmod 3)  \\
|1\rangle^{A_{k_2}^I} \langle   1 |^{A_{k_1}^O}   & \text { if } k_2=k_1+2 & (\bmod 3)
\end{array}, \right. \\
...
|0\rangle^{A_{k_3}^I}|1\rangle^{\alpha_3} \langle   0 |^{A_{k_2}^O}+\mid 1 \rangle^{A_{k_3}^I}|0\rangle^{\alpha_3} \langle   1 |^{A_{k_2}^O}   & \text { if } k_2=k_1+2 \quad(\bmod 3)
\end{array},  \right. \\
& V_{ \{k_1, k_2 \}, k_3}^F=\mathbb{1}^{A_{k_3}^O \alpha_3  \rightarrow F^t \alpha_F } \otimes |k_3 \rangle^{F^c},
\end{aligned}
$$
>   
...
$$
\begin{aligned}
&V_{\emptyset, \emptyset}^{\rightarrow k_1}:=\mathbb{1}^{P_t \rightarrow A_{k_1}^I} \otimes\left\langle\left. k_1\right|^{P_c},\right.\\
&V_{\emptyset, k_1}^{\rightarrow k_2}:=\left(\mathbb{1}^{A_{k_2}^I} \otimes\left\langle\left.\sigma_{\left(k_1, k_2\right)}\right|^\alpha\right) \mathcal{V}_{k_1},\right.\\
& V_{\left\{k_1\right\}, k_2}^{\rightarrow k_3}:=\mathcal{V}_{k_3}^{\prime}\left(\mathbb{1}^{A_{k_2}^O} \otimes\left|\sigma_{\left(k_1, k_2\right)}\right\rangle^{\alpha^{\prime}}\right),\\
& V_{\left\{k_1, k_2\right\}, k_3}^{\rightarrow F }:=\mathbb{1}^{A_{k_3}^O \rightarrow F_t} \otimes \mathbb{1}^{\alpha_3 \rightarrow \alpha_F} \otimes\left|k_3\right\rangle^{F_c} .
\end{aligned}
$$
>
...
>
$$
= \sum_{k_1} \text{tr}_{A_{k_1}^I} |\mathbb{1}\rangle \langle \mathbb{1}|_{P_t A_{k_1}^I} \otimes |k_1\rangle \langle k_1|_{P_c} = \mathbb{1}_{P_t} \otimes \mathbb{1}_{P_c} = \mathbb{1}_P.
$$
...
>
$$
|V_{\emptyset,k_1}^{\to k_2}\rangle_{A_{k_1}^OA_{k_2}^I} = \mathbb{1}_{A_{k_1}^I} \otimes \langle \sigma_{(k_1,k_2)}|_\alpha \otimes \mathbb{1}_{A_{k_1}^O} |V_{k_1}\rangle_{A_{k_2}^I \alpha A_{k_1}^I} = \langle \sigma_{(k_1,k_2)}| \mathcal{V}_{k_1}\rangle_{A_{k_2}^I A_{k_1}^O}
$$
>
>
$$
...
>
$$
= | w_{(1)}\rangle \langle  w_{(1)}| \otimes \mathbb{1}_{A_1^o}
$$
...
>
$$
|V_{\mathcal{K}_2 \setminus \{k_2\}, k_2}^{\to k_3}\rangle = (\mathbb{1}_{A_{k_2}^O}\otimes \mathcal{V}_{k_3}^\prime   ) |\mathbb{1}\rangle_{A_{k_2}^O A_{k_2}^O} \otimes |\sigma_{(k_1, k_2)}\rangle_{\alpha'}
$$
>
$$
...
>
$$
= \delta_{k_2 q_2} \mathbb{1}_{A_{k_2}^o} \quad (2)
$$
>   综合 (1), (2)，有
>
$$
\sum_{k_3 \notin \mathcal{K}_2} \text{tr}_{A_{k_3}^I \alpha_3} | w_{(\mathcal{K}_2, k_3)}\rangle \langle  w_{(\mathcal{K}_2, k_3)}| = \sum_{k_2 \in \mathcal{K}_2} | w_{(\mathcal{K}_2 \setminus \{k_2\}, k_2)}\rangle \langle  w_{(\mathcal{K}_2 \setminus \{q_2\}, q_2)}| \otimes \mathbb{1}_{A_{k_2}^o}
$$
...
>   =& \sum|V_{\emptyset,\emptyset}^{\to k_1}\rangle\otimes|V_{\emptyset,k_1}^{\to k_2}\rangle\otimes|V_{\{k_1\},k_2}^{\to k_3}\rangle|\mathbb{1}\rangle_{A_{k_3}^O F_t}|k_3\rangle_{F_c}\\
>   =& \sum_{k_3}\sum_{k_1k_2}| w_{(k_1,k_2,k_3)}\rangle|\mathbb{1}\rangle|k_3\rangle \\
>   =&\sum_{k_3} | w_{(\mathcal{N}\setminus  k_3,k_3)}\rangle|\mathbb{1}\rangle|k_3\rangle\,
>   \end{align*}$$
>   上式已经利用了
>
$$
...
$$
\begin{aligned}
\left|V_{\{k_1\}, k_2}^{\rightarrow k_3}\right\rangle *\left|V_{\left\{k_1, k_2\right\}, k_3}^{\rightarrow F}\right\rangle & =\left(\left|V_{\left\{k_1\right\}, k_2}^{\rightarrow k_3}\right\rangle_{A_{k_3}^I \alpha_3 A_{k_2}^O}^{T_{\alpha_3}} \otimes \mathbb{1}_{F_t F_c \alpha_F}\right)\left(|\mathbb{1}\rangle_{A_{k_3}^O F_t} \otimes|\mathbb{1}\rangle_{\alpha_3 \alpha_F} \otimes\left|k_3\right\rangle_{F_c}\right) \\
& =\mathbb{1}^{\alpha_3 \rightarrow \alpha_F}\left|V_{\left\{k_1\right\}, k_2}^{\rightarrow k_3}\right\rangle_{A_{k_3}^I \alpha_3 A_{k_2}^O}|\mathbb{1}\rangle_{A_{k_3}^O F_t}\left|k_3\right\rangle_{F_c}
\end{aligned}
$$
>   保迹条件写为
>  $$ \begin{align*}
   &\mathrm{tr}_{F\alpha_F}| w(\mathcal{N},F)\rangle\langle  w(\mathcal{N},F)|=\mathrm{tr}_{F\alpha_F}\left(\sum_{k_3,q_3} | w_{(\mathcal{N}\setminus  k_3,k_3)}\rangle\langle  w_{(\mathcal{N}\setminus  q_3,q_3)}| \otimes|\mathbb{1}\rangle_{A_{k_3}^OF_t}\langle\mathbb{1}|\otimes|k_3\rangle_{F_c}\langle q_3|\right)\\
   =&\mathrm{tr}_{\alpha_F}| w_{(\mathcal{N}\setminus  k_3,k_3)}\rangle\langle  w_{(\mathcal{N}\setminus  q_3,q_3)}|\otimes\mathbb{1}_{A_{k_3}^O}\otimes\delta_{k_3q_3}\\
   =&\mathrm{tr}_{\alpha_F}| w_{(\mathcal{N}\setminus  k_3,k_3)}\rangle\langle  w_{(\mathcal{N}\setminus  k_3,k_3)}|\otimes\mathbb{1}_{A_{k_3}^O}
   \end{align*}$$
>   
...
$$
\begin{aligned}
|\widetilde{V}_1\rangle &= \sum_{k_1} |\mathbb{1}\rangle^{P_t A_{k_1}^I} |k_1\rangle^{P_c} | \phi, k_1 \rangle^{C_1},\\
\mathrm{tr}_{A_1^IC_1}  | \widetilde{V}_1 \rangle\langle \widetilde{V}_1|&= \mathbb{1}_{P_t}\otimes \mathbb{1}_{P_c}\otimes \mathbb{1}_{C_1}
\end{aligned}
$$
> -
$$
...
&= \mathbb{1}_{A_1^o} \otimes \mathbb{1}_{C_1'}.
\end{aligned}
$$
> -
$$
\begin{aligned}
|V_3\rangle &= \sum_{k_1,k_2,k_3} \mathbb{1}_{A_3^o} \otimes \mathcal{V}_3^\prime  |\mathbb{1}\rangle_{A_3^o A_3^o} |\sigma_{(k_1,k_2)}\rangle^{\alpha'} |\{k_1\}, k_2\rangle^{C_2^\prime} |\{k_1,k_2\}, k_3\rangle^{C_3}, \\
\mathrm{tr}_{\alpha_2A_3^I C_3} | \widetilde{V}_3 \rangle\langle \widetilde{V}_3|  &= \sum_{\{k,k_2\} \atop =\{q_1,q_2\}} \mathrm{tr}_{\alpha_3 A_3^I} \left( \mathbb{1}_{A_3^o} \otimes \mathcal{V}_3^\prime  \right) |\mathbb{1}\rangle_{\alpha_3^o A_3^o} \otimes |\sigma_{(k,k_2)}\rangle_{\alpha'} \langle \mathbb{1} |_{\alpha_3^o A_3^o} \langle \sigma_{(q_1,q_2)} | \left( \mathbb{1}_{A_3^o} \otimes \mathcal{V}_3^{\prime\dagger} \right) \otimes |\{k_1\}, k_2\rangle\langle\{q_1\}, q_2| \\
...
&= \mathbb{1}_{A_2^o} \sum_{\{k_1,k_2\} \atop =\{q_1,q_2\}} \langle \sigma_{(q_1,q_2)} | \sigma_{(k_1,k_2)} \rangle |\{k_1\}, k_2\rangle\langle\{q_1\}, q_2| \\
&= \mathbb{1}_{A_2^o} \otimes \mathbb{1}_{C_2'}.
\end{aligned}
$$
> -
$$
...
&= |\mathbb{1}\rangle^{A_2^o F_t} \otimes |\mathbb{1}\rangle \sum_{k_3} |k_3\rangle^{F_c} |\{k_1,k_2\}, k_3\rangle^{C_3'}, \\
\mathrm{tr}_{\alpha_F F_c} |V^4\rangle\langle V^4| &= \mathbb{1}_{A_3^o} \otimes \mathbb{1}_{\alpha_3} \otimes \mathbb{1}_{C_3'}.
\end{aligned}
$$
>
>
...
$$
\begin{aligned}
& \rho^{\mathsf{DS}_{\mathrm{eff}}}=|\psi\rangle _{P_c}\langle \psi| \star |w_{\mathsf{DS}}\rangle \langle w_{\mathsf{DS}}| \star  \rho^\mathsf{Dp} \star  \rho^\mathsf{Dp} \star  \rho^\mathsf{Dp} \\  = & \frac{1}{8}\left( (|0 0\rangle\langle 0 0|+|1 1\rangle\langle 1 1|)_{P_t F_t}\otimes|\psi\rangle\langle\psi|_{F_c}+(\mathbb{1}_{P_t F_t}+\frac{1}{3}|0 1\rangle\langle 0 1|+|1 0\rangle\langle 1 0|)\otimes\mathbb{1}_{F_c} \right)
\end{aligned}
$$
...
>
$$
\mathrm{tr}_{F_c}(\rho^{\mathsf{DS}_{\mathrm{eff}}}) = \frac{\mathbb{1}_{P_t  F_t}}{2}
$$
...
\begin{aligned}
V_{\emptyset, \emptyset}^{\rightarrow k_1}&=\mathbb{1}^{P_t \rightarrow A_{k_1}^I} \otimes\left\langle\left. k_1\right|^{P_c},\right.\\
V_{\emptyset, k_1}^{\rightarrow k_2} &=\left(\mathbb{1}^{A_{k_2}^I} \otimes\left\langle\left.\sigma_{\left(k_1, k_2\right)}\right|^\alpha\right) \mathcal{V}_{k_1}^{(1)},\right.\\
V_{\left\{k_1\right\}, k_2}^{\rightarrow k_3}& =\mathcal{V}_{k_3}^{(3)}\left(\mathbb{1}^{A_{k_2}^O} \otimes\left|\sigma_{\left(k_1, k_2\right)}\right\rangle^{\alpha^{\prime}}\right),\\
V_{\{k_1, k_2\}, k_3} ^{\to k_4} &= \left( \mathcal{V}_{k_4}^{(4)} \left( \mathbb{1}^{A_{k_3}^O} \otimes \left| \theta(\{k_1, k_2\}, k_3) \right\rangle^{\alpha_4^{(1)\prime}} \right) \right) \otimes \mathbb{1}^{\alpha_3 \to \alpha_4^{(2)}}  \\
V_{\{k_1, k_2, k_3\}, k_4} ^{\to F} &= \mathbb{1}^{A_{k_4}^O \to F} \otimes \mathbb{1}^{\alpha_4^{(1)\prime} \alpha_4^{(2)} \to \alpha_F} \otimes\left| k_4 \right\rangle^{F_c}
\end{aligned}
$$
>   其中
...
$$   \begin{align*}
  ...
  &= \sum_{\substack{\{k_1, k_2\}, k_3 \\ \{q_1, q_2\}, q_3 \\ \{k_1, k_2, k_3\} = \{q_1, q_2, q_3\}}} |i\rangle\left< j \right| \mathrm{tr} \left({\mathcal{V}_{k_4}^{(4)}}  \left| i \right> \left< j \right| \otimes \left| \theta(\{k_1, k_2\}, k_3) \right> \left< \theta(\{q_1, q_2\}, q_3) \right| \mathcal{V}_{k_4}^{\prime \dagger}\right) \left| \{k_1, k_2\}, k_3 \right> \left< \{q_1, q_2\}, q_3 \right| \otimes |\mathbb{1}\rangle \left\langle \mathbb{1} \right| \\
  &= \sum \mathbb{1}_{A_3^o} \mathbb{1}_{\alpha_3} \left< \theta(\{k_1, k_2\}, k_3) \right| \left. \theta(\{q_1, q_2\}, q_3) \right> \left| \{k_1, k_2\}, k_3 \right> \left< \{q_1, q_2\}, q_3 \right| \\
  &= \sum \mathbb{1}_{A_3^o} \mathbb{1}_{\alpha_3} \delta_{k_3 q_3} \delta_{\{k_1, k_2\}, \{q_1, q_2\}} \left| \{k_1, k_2\}, k_3 \right> \left< \{q_1, q_2\}, q_3 \right| \\
  &= \mathbb{1}_{A_3^o} \mathbb{1}_{\alpha_3} \mathbb{1}_{C^3}
  \end{align*}$$
...
V_{\left\{k_1\right\}, k_2}^{\rightarrow k_3}& =\mathcal{V}_{k_3}^{(3)}\left(\mathbb{1}^{A_{k_2}^O} \otimes\left|\sigma_{\left(k_1, k_2\right)}\right\rangle^{\alpha_3^{\prime}}\right),\\
V_{\{k_1, k_2\}, k_3} ^{\to k_4} &= \left( \mathcal{V}_{k_4}^{(4)} \left( \mathbb{1}^{A_{k_3}^O} \otimes \left| \theta(\{k_1, k_2\}, k_3) \right\rangle^{\alpha_4^{\prime}} \right) \right) \otimes \mathbb{1}^{\alpha_3 \to \beta_3}  \\
V_{\{k_1, k_2, k_3\}, k_4} ^{\to F} &= \mathbb{1}^{A_{k_4}^O \to F} \otimes \mathbb{1}^{\alpha_4^{(1) } \alpha_4^{(2)} \to \alpha_F} \otimes\left| k_4 \right\rangle^{F_c}
\end{aligned}
$$
...
V_{ \{ k_1, \ldots, k_{n-1} \} , k_n }^{ \to k_{n+1} }&  = \left( \mathcal{V}_{k_{n+1}}^{ (n+1) } \left( \mathbb{1}^{A_{k_n}^o} \otimes |\sigma^{ (n-1) } \left( \{ k_1, \ldots, k_{n-1} \}, k_n \right)\rangle^{ \beta_{n+1}' }  \right) \right) \otimes \mathbb{1} ^{\alpha_n \to  \gamma_n},~n=2,\ldots ,N-1\\
V_{ \{ k_1, \ldots, k_{N-1} \} , k_N }^{\to {F}}&  = \mathbb{1}^{A_{k_N}^{O} \to F} \otimes \mathbb{1}^{\alpha_N \rightarrow F} \otimes |k_N\rangle ^{F_c}
\end{aligned}
$$
...
$$
|W\rangle =\sum_\pi |\pi\pi\rangle_{Q^\prime Q^*} |\mathbb{1}\rangle^{\otimes N+1} _{S^*A^{\pi(1)}\cdots A^{\pi(N)}S^\prime}
$$
...
$$
|W,s\rangle  = |W\rangle ^{\top_Q} (\mathbb{1}\otimes |s\rangle ) = \sum_\pi s_\pi|\pi \rangle _{Q^\prime } |\mathbb{1}\rangle^{\otimes N+1} _{S^*A^{\pi(1)}\cdots A^{\pi(N)}S^\prime}
$$
完全退极化信道的蔡氏矩阵正比于单位阵，$\rho^\mathsf{Dp}=\frac{\mathbb{1}}{d}$，如果插入的$N$个信道都是完全退极化信道，那么得到的等效过程为(符号偏迹.nb)
$$
\begin{aligned}
...
&1 \overset{\alpha}{\sim} N+1
 \\[6pt]
d^{c_\alpha}\,\lvert \mathbb{1}\rangle\langle \mathbb{1} \rvert,
...
$$
\hat{X}^\mathsf{Dp}= \frac{1}{d}\sum_\pi |s_\pi|^2 |\pi\rangle \langle \pi | \otimes \mathbb{1} +\frac{1}{d^2}\sum_{\theta\neq \pi } s_\pi s_\theta^*|\pi\rangle\langle\theta| \otimes |\mathbb{1}\rangle\langle \mathbb{1}|
$$
