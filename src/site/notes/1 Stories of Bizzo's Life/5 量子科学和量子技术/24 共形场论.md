...
$$
\ket{\Phi^+} = \sum_{j=1}^d \frac{1}{\sqrt{d}} \ket{jj} \text{ 和 } \ket{A} \equiv (A \otimes \mathbb{1}) \ket{\Phi^+}.
$$
将量子态与$A$联系起来。
\end{mybox}
...
$$\begin{align}
\ket{A_{in}}
&\rightarrow \hat{A} \otimes \mathbb{1} \ket{\Phi^+} \\
\ket{A_{out}} &\rightarrow \mathbb{1} \otimes \hat{\bar{A}} \ket{\Phi^+}.
\end{align}$$
