...
### 讲义
\begin{itemize}
    \item [Waterloo大学](https://cs.uwaterloo.ca/~watrous/TQI/TQI.4.pdf)。
\end{itemize}
...
> - 给定初始时刻$t_0$，设量子系统的演化由时间$t$索引的一族量子信道$\{\mathcal{E}_{(t,t_0)}\mid t_0\leqslant t\leqslant t_2\}$描述，满足$\mathcal{E}_{(t_0,t_0)}=\mathbb{1}$，
...
设$\{F_i\}$是$B(\mathsf{H})$上的一组标准正交基，$F_0=\mathbb{1}/\sqrt{d}$，其他算符无迹，$B(\mathsf{H})$上一般的线性映射写成
$$
...
以及
$$
V_k \mapsto E_k= V_k+\chi_k \mathbb{1}, \quad \hat{H} \mapsto K= \hat{H}-\frac{\mathrm{i}}{2} \sum_{k=1}^K\left(\chi_k^* V_k\gamma_k-\text { h.c. }\right) .
$$
...
$$
噪声空间为$\mathsf{E}\equiv \mathsf{L}\mathsf{L}+\mathsf{L}^\dagger\mathsf{L}=\mathsf{L}^2+\mathsf{L}^\dagger \mathsf{L}$，而$\{E_i\}$是该空间上的一组标准正交基，构造恢复信道时根据$\mathcal{E}=\{E_i \}$来构造。由于$\mathbb{1}=L_0\in \mathsf{L}$，因此$\mathsf{L}$是$\mathsf{E}$的子空间。

利用信道纠缠度刻画误差然后证明必要性可能相当费劲[[beny2010GeneralConditionsApproximate\|beny2010GeneralConditionsApproximate]]（是吗？）。

为了简化记号，定义$L_0=\mathbb{1}$。
...
> [!proof]
> - 不失一般性，可以设$L_0=\mathbb{1}$，其他的$L_i$是无迹的。取恢复信道
>
$$
...
$$
>   这直接排除了所有满足$E^2=\mathbb{1}$形式的噪声。很多我们关心的噪声，例如单比特泡利算符都被排除在外，很多纠错码不满足该性质也能够纠正二阶错误。
>
>
...
$$
>   幺正变换满足$UU^\dagger=VV^\dagger \otimes WW^\dagger=\mathbb{1}$，其分量形式写为
>
$$
\begin{aligned}
...
任何测量都可以通过主方程的长时间演化近似实现，设$\mathcal{L}(\rho)=\sum_{i}P_i\rho P_i-\rho$，则$\mathcal{L}^{n}=(-1)^{n+1} \mathcal{L},~\forall n>0$，因此$\exp(\mathcal{L}t)=\mathbb{1}+\mathcal{L}(1-\cosh t+\sinh t)$，进而
$$
...
& P_k P_l=\delta_{k l} P_k, \operatorname{tr}\left(P_k\right)=d_k, \sum_k P_k=\mathbb{1}
\end{aligned}
$$
...
& \mathcal{P}_k \mathcal{P}_l=\delta_{k l} \mathcal{P}_k, \operatorname{tr}\left(\mathcal{P}_k\right)=d_k, \sum_k \mathcal{P}_k=\mathbb{1}
\end{aligned}
$$
...
$$
其中$n=d^2-1$。正交性条件写为$S^{-1}S=\mathbb{1}$。
...
### 主方程的微观推导\label{sec:mastermicor}
...
& E_1(t)=\sqrt{\frac{1+e^{\Gamma(t)}}{2}} \mathbb{1}  \quad E_2(t)=\sqrt{\frac{1-e^{\Gamma(t)}}{2}} \sigma_z
\end{aligned}
$$
...
{ #4}
\left(\frac{\Delta t \omega_k }{2}\right) \sin
{ #4}
(\Delta t N \omega_k ) \geqslant 0
$$
...
& =\gamma D\left[\sigma_{-}\right](\rho)-i \Delta \omega_0\left(\rho\left(\sigma_{+} \sigma_{-}-\frac{1}{2} \mathbb{1}\right)-\left(\sigma_{+} \sigma_{-}-\frac{1}{2} \mathbb{1}\right) \rho\right) \\
...
双链：[经典涨落定理](onenote:[[[[[[[[[[[[[[[[[[[[[[[[复杂系统\|[[[[[[[[[[[[[[[[[[[[[[复杂系统]]]]]]]]]]]]]]]]]]]]]]]]与统计物理.one#热力学和经典统计力学&section-id={BE9F92D7-4720-417F-9580-E01650DBDE7E}&page-id={C93F15B4-0B59-4DE2-9228-0E354432DFC4}&object-id={3EECD17C-96F7-46B7-BAA1-5CB3C7111E63}&16&base-path=https://d.docs.live.net/b881ef1fd2bef8b4/文档/卡片笔记写作/(2)%20%5bStory%20of%20Bizzo's%20Life%5d)Crooks
