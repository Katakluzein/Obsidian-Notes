...
$$
可知，$\pi$是正的，因此保厄米且保矩阵乘法的映射都是正的。根据同态的定义(Moretti, 2017, Def.2.24)可知，$\pi(\mathbb{1})=\mathbb{1}$，$\|\pi\|\leqslant 1$，根据Russo-Dye定理可知，$\pi$是收缩的。
...
- 利用unital \*-同态的性质，可以从Stinespring表示得到Kraus算符和表示，设$\mathfrak{A}$是矩阵代数$B(\mathsf{H}_1)\equiv M_n(\mathbb{C})$，$\phi:B(\mathsf{H}_1)=\mathfrak{A}\rightarrow B(\mathsf{H}_2)=M_k(\mathbb{C})$，则$\pi(\mathfrak{A})$是一个因子(是吗？是否需要排除$\pi(\mathfrak{A})=\mathbb{1}$的特殊情形？)，在幺正等价的意义下，存在$C^*$-代数的分解$\pi(\mathfrak{A})= \mathbb{1}_l \otimes \mathfrak{A} \in B(\mathsf{K})$（为什么？），空间的分解$\mathsf{K}=\bigoplus_{i=1}^l \mathbb{C}_i^n,~l\leqslant nk$，且满足$P_i\pi(a)P_i=a$，即$\pi$在子代数$P_i \mathfrak{A} P_i$上的限制是恒等表示。定义$V_i=P_iV$可得
  $$
...
- 同构只要证明逆射存在。推论已证，$\mathcal{E}$全正当且仅当$\mathsf{E}$半正定。$\mathcal{E}$保迹当且仅当$\mathrm{tr}_B(\mathsf{E})=\mathbb{1}_{A^*}$。
...
- 另外我们还可以用超线性映射来刻画这个保迹条件，设
  ...
  $$
  其中${}_i \mathsf{E}=\mathrm{tr}_i(\mathsf{E})\otimes \mathbb{1}_i/d_i$是子系统$i$的完全退极化信道，可以证明$\mathcal{P}$是一个算符系统上的正交投影，则
  $$
...
$$
D(\rho \ || \ \sigma) – D(\mathcal{N}(\rho) \ || \ \mathcal{N}(\sigma)) \geq 0
$$
更高阶的数据处理不等式的推广，参见(Fundamental Limitations on the Recoverability of Quantum Processes)。

利用数据处理不等式可以证明：$S(\mathcal{N}(\rho)) \geq S(\rho)$
