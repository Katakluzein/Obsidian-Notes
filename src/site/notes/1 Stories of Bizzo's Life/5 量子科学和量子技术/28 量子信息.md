...
> [!quote]  I take the view that we all have permission to be a little baffled by quantum information science and algorithmic information theory. 
> —— James Gleick 
...
参考（[[ChenYun2015XinXiLunYuBianMa\|ChenYun2015XinXiLunYuBianMa]] (4.2.6)）。互信息$I(X ; Y)$ 是关于信源概率分布 $\left\{p_x\right\}$ 和信道转移概率分布 $\left\{f_{yx}\right\}$ 的二元函数，当信道特性 $\left\{f_{yx}\right\}$ 固定后，$I(X ; Y)$ 随信源概率分布 $\left\{p_x\right\}$ 的变化而变化。调整 $\left\{p_x\right\}$ ，在接收端就能获得不同的[[信息量\|信息量]]。由互信息的性质已知，$I(X ; Y)$ 是 $\left\{p\left(a_i\right)\right\}$ 的上凸函数。因此总能找到一种概率分布 $\left\{p\left(a_i\right)\right\}$（即某一种信源），使信道所能传送的信息率为最大。我们定义这个最大的信息传输率为**信道容量**，记为 $C$ ，
$$
C=\max _{p_x} I(X ; Y)
$$
粗略地说，如果调用$n$次二元信道能够可靠地传输$nC$比特信息，那么我们称这个二元信道的容量为$C$（[[nielsen2001quantum\|nielsen2001quantum]] Sec.12.3.1）。

经典信道和量子信道的关系：经典信道和随机变量的确定性的函数都可以被视为特殊的量子信道，只需要将每条信息编码为Hilbert基矢量即可[[Embedding cyclic information-theoretic structures in acyclic spacetimes no-go results for indefinite causality\|embding ICO notation 5]]。
## 量子信道 Quanutum Channel 
### 保厄米、正、全正映射


> [!definition] 保厄米性 (Hermiticity-preserving) / 保-dagger 映射
> 如果 `*` 表示厄米共轭（Hermitian conjugate），那么一个映射 $\phi$ 保持厄米性等价于以下条件：
> $$
> \phi(X)^* = \phi(X^*)
> $$

> [!definition] 全正映射 (Completely Positive Map) {\cite[Chap.3]{paulsen2002completely}}
>
...
> [!note]
> - **问题**：正映射 (positive map) 是否有像全正映射一样，不需要遍历所有维度 $n$ 就能使用的简单判据？或许可以参考文章[[hou2015NewCriterionSpecial\|hou2015NewCriterionSpecial]]。
>
>
>   全正映射的迹一定是正的，但正映射的迹不一定是正的。参见[这个 Stack Exchange 帖子](https://quantumcomputing.stackexchange.com/questions/38054/is-the-trace-of-a-positive-map-always-positive)。对于线性泛函（linear functional）来说，全正和正是等价的（见 [[paulsen2002completely\|paulsen2002completely]] 命题3.8）。
>
>   **问题**：是否存在物理上可以实现的、正但非全正的映射？

  $\mathfrak{A} \otimes M_n(\mathbb{C})  \rightarrow M_n(\mathfrak{A}), \sum_{ij}A_{ij}\otimes E_{ij} \mapsto (A_{ij})$是${}^*$-同构，通常直接记作$(A_{ij})=\sum_{ij}A_{ij}\otimes E_{ij}$，其中$E_{ij}$是matrix unit。因此有全正的等价定义：$\phi$是$n$-正的$\Leftrightarrow \phi\otimes \mathbb{1}_{n\times n}$是正的，其中$\mathbb{1}_{n\times n}$是$n$阶单位矩阵。必要性：设$\phi$是$n$-正的，任取正的$x=\sum_{ij}A_{ij}\otimes E_{ij} \in \mathfrak{A}\otimes M_n(\mathbb{C})$，有$(\phi(A_{ij}))=\sum_{ij}\phi(A_{ij})\otimes E_{ij}=(\phi\otimes \mathbb{1})(\sum_{ij}A_{ij}\otimes E_{ij})$是正的，即$\phi\otimes \mathbb{1}$是正的。充分性类似。

$\pi:\mathcal{A}\rightarrow \mathcal{B}$是${}^*$-同态，则由
$$
\pi\left(A^{*} A\right)=\pi\left(A^{*}\right) \pi(A)=\pi(A)^{*} \pi(A) \geq_{\mathfrak{B}} 0
$$
可知，$\pi$是正的，因此保厄米且保矩阵乘法的映射都是正的。根据同态的定义{（[[Moretti2017\|Moretti2017]] Def.2.24）}(定义\ref{def:staralgebrahom})可知，$\pi(\mathbb{1})=\mathbb{1}$，$\|\pi\|\leqslant 1$，根据定理\ref{th:russodye}可知，$\pi$是收缩的。

    
    
 $\pi$是全正的，即$\pi_n:M_n(\mathfrak{A})\rightarrow M_n(\mathfrak{B})$是正的。

  

 根据引理{（[[paulsen2002completely\|paulsen2002completely]] Lm.3.13）}，任取$P=B^{*} B \in M_n(\mathfrak{A})$，都有
$$
...
> - 设$\Phi:M_n(\mathbb{C})\rightarrow M_k(\mathbb{C})$，是正映射，则$\|\Phi\|=\|\Phi(\mathbb{1})\|$，在$M_{n,k}(\mathbb{C})$上使用$\infty$-范数。

> [!proof]
> - 参见{（[[bhatia2009PositiveDefiniteMatrices\|bhatia2009PositiveDefiniteMatrices]] Th.2.3.7）}
...
> [!proof]
> - 参见{（[[bhatia2009PositiveDefiniteMatrices\|bhatia2009PositiveDefiniteMatrices]] Th.2.6.7）}

> [!note]
> - 结合上述两个定理可知，unital映射是正的当且仅当$\|\Phi\|=1$。

> [!theorem] 保迹保厄米映射的正性的充要条件
{ #th-th-prehpretiff}
...
>
>   从这个定理可以立即推论量子信道不增加迹距离，或者说迹距离关于量子信道是单调的，参见定理\ref{prop:tracedischnmono}。
>
>   注意收缩性不是严格的，可能取得等号。

综合上面的两个定理可以看出，正性在某种意义下就是收缩性。
> [!definition] 不可约正映射[[lardizabal2022MeanHittingTime\|lardizabal2022MeanHittingTime]]
> - \label{def:irreduciblemap}
>   设$\Phi: M_n \rightarrow M_n$是正的，$P$是正交投影算符，如果
$$
\Phi\left(P M_n P\right) \subset P M_n P
$$
>   蕴含$P=0$或$P=\mathbb{1}$，则称$\Phi$是**不可约**的。
...
证明参见图片
[[2024-03-10-17-34-51 1.png]]

\begin{trick}
    利用上述引理，将全正性的证明约化为$(\pi(a_i^*a_j))$的正性的证明。这个[[技巧\|技巧]]在Choi（[[paulsen2002completely\|paulsen2002completely]] Th.3.14 ）的证明中也发挥了作用。
\end{trick}

好的，这是对 Stinespring 扩张定理的翻译和整理。

> [!theorem] Stinespring 扩张定理 (Stinespring’s Dilation Theorem) (参见 [[paulsen2002completely\|paulsen2002completely]] Th. 4.1)
>
...
$$
>   可知，$\phi$是\myemph{全正}的蕴含了$\langle\cdot, \cdot \rangle$是半正定的。其中$\langle, \rangle_{\mathsf{H}^{(n)}}$是$\mathsf{H}^{(n)}$上自然的内积。
>   根据半正定厄米对称的双线性形式的Cauchy-Schwartz不等式
>
$$
...
$$
>
>   逆命题：任何$*$-同态都是全正的，这在已经知道式\eqref{eq:pipositive}情况下是显然的。

> [!note]
> - 定理中的信道是海森堡表象中的，即$\phi$实际上作用在effect上而不是密度算符上。
>
>
>   定理本身没有对$V$是否为等距做出约束。当$\phi$是unital，即$\phi^\dagger$保迹时，$V$是等距算符(isometry)，因此$\mathsf{H}= V \mathsf{H}$可以视为$\mathsf{K}$的子空间。设$T\in B(\mathsf{K})$，则算符$V^\dagger T|_{B(\mathsf{H})}$被称为$T$到$\mathsf{H}$上的压缩}(compression){（[[paulsen2002completely\|paulsen2002completely]] Th.4.1）}。做分解$\mathsf{K}=\mathsf{H}\oplus \mathsf{H}^\perp$，将$T$视为2阶块状矩阵，则$T$到$\mathsf{H}$上的压缩就是左上角的块。unital全正算符的Stinespring膨胀可以表述为：$B(\mathsf{H})$上的全正算符都是某个包含$\mathsf{H}$的空间$\mathsf{K}$上的$*$-同态到$\mathsf{H}$上的压缩。
>
...
>   利用unital \*-同态的性质，可以从Stinespring表示得到Kraus算符和表示(定理\ref{th:operatorsumkarus})，设$\mathfrak{A}$是矩阵代数$B(\mathsf{H}_1)\equiv M_n(\mathbb{C})$，$\phi:B(\mathsf{H}_1)=\mathfrak{A}\rightarrow B(\mathsf{H}_2)=M_k(\mathbb{C})$，则$\pi(\mathfrak{A})$是一个因子(是吗？是否需要排除$\pi(\mathfrak{A})=\mathbb{1}$的特殊情形？)，在幺正等价的意义下，存在$C^*$-代数的分解$\pi(\mathfrak{A})= \mathbb{1}_l \otimes \mathfrak{A} \in B(\mathsf{K})$（为什么？），空间的分解$\mathsf{K}=\bigoplus_{i=1}^l \mathbb{C}_i^n,~l\leqslant nk$，且满足$P_i\pi(a)P_i=a$，即$\pi$在子代数$P_i \mathfrak{A} P_i$上的限制是恒等表示。定义$V_i=P_iV$可得
>
$$
...
$$
>   j
>   Kraus算符和的幺正自由度是一种特殊的Stinespring幺正自由度，取最简单的unital $*$-同态，将$l$个输入矩阵放在对角线上制作成块对角阵，$\pi(a)=\mathbb{1}_l\otimes a=\mathrm{diag}(a,\ldots,a)\in M_{nl}(\mathbb{C})$，设Kraus算符的幺正变换为$W=UV$，其中$U=(u_{ij})\otimes \mathbb{1}_n$
>
$$
\left(\begin{array}{c}
W_1 \\
...
u_{11} \mathbb{1}_n & & u_{1l} \mathbb{1}_n\\
& \ddots & \\
u_{l1} \mathbb{1}_n & & u_{ll} \mathbb{1}_n
...
$$
>
>   \myemph{问题}：如果取最简单unital *-同态，是否能够利用公式$\phi(a)=V^\dagger \pi(a)V$制作一个新的Fisher信息量公式？公式中要包含关于$V$的最小化。
>
>   \myemphg{问题}：Stinespring表示和系统-环境耦合表示的关系是什么？{（[[Encyclopedia2006\|Encyclopedia2006]] p.474）}
...
>
$$
\langle \phi(a),\rho\rangle=\mathrm{tr}_{\mathsf{H}_2}\left(V^\dagger (\mathbb{1}_l\otimes a) V\rho\right)
=\mathrm{tr}_\mathsf{K}\left((\mathbb{1}_l\otimes a) V\rho V^\dagger\right)=\mathrm{tr}_{\mathsf{H}_1}\left(a \mathrm{tr}_\mathsf{A}(V\rho V^\dagger)\right)
...
> [!definition] Quantum Channel and Quantum Operation
> - \label{def:quantumchannel}
>   保迹的全正的线性映射$\mathcal{E}:B(\mathsf{H}_1)\rightarrow B(\mathsf{H}_2)$称为(薛定谔表象的)**量子信道**(quantum channel)，
>   如果条件放宽为迹小于等于1，那么称该映射为**量子操作**(quantum operation){（[[nielsen2001quantum\|nielsen2001quantum]] Sec.8.2.4）}。

> [!note]
> - 为什么在定义这个概念的时候要求迹不能大于1？
>
>   量子操作可纠错的必要条件定理\ref{lm:necessaryqec}{（[[Scherer2019\|Scherer2019]] Th.7.22）}是
>
$$
\operatorname{tr}(\mathcal{E}(\rho))=\text{const.}=\kappa_{\mathcal{E}} \in[0,1] \quad \forall \rho \in D\left(\mathsf{H}^c\right)
$$
>   其中，$\mathsf{H}^c$是码空间。
>
...
>
>   量子信道的三种等价刻画，参见图\ref{fig:eqquantumchannel}：
>   \begin{figure}[htbp]
>   \centering
>
>
...
>   & {\text{公理化定义}} \\
>   \\
>   {\text{Kraus算符和表示}} && {\text{系统环境耦合表示}}
>   \arrow[Rightarrow, 2tail reversed, from=3-1, to=3-3]
>   \arrow[Rightarrow, 2tail reversed, from=3-1, to=1-2]
>   \arrow[Rightarrow, 2tail reversed, from=1-2, to=3-3]
>   \end{tikzcd}
>   \end{document}
>   ```
>
...
>
>   \caption{ 量子信道的三种等价刻画 }
>   \label{fig:eqquantumchannel}
>   \end{figure}
>
> - 带有物理动机的公理(公理化定义)
>   \begin{itemize}
> - 线性的动机：量子力学大多数时候都是线性的。
...
> - \includegraphics[width=0.7\textwidth]{image//2024-03-10-16-16-34.png}
...
> [!lemma] 蔡氏定理(Choi)
> - \label{th:choistheorem}
>   $\mathfrak{B}$是$C^*$-代数，$\mathcal{E}:M_n\rightarrow \mathfrak{B}$是线性映射，下列命题等价
>   
> - $\mathcal{E}$全正。
> - $\mathcal{E}$是$n$-正的。
> - $\left(\mathcal{E}\left(E_{i, j}\right)\right)_{i, j=1}^{n}$ 在 $M_{n}(\mathfrak{B})$中是正的。
>   
...
>
>
>   $(3)\Rightarrow(1)$只要证明$\mathfrak{B}=B(\mathsf{H})$的情形，设$|x_{1}\rangle, \ldots, |x_{k}\rangle\in \mathsf{H}$，$B_{1}, \ldots, B_{k}\in M_n$ ，根据引理{（[[paulsen2002completely\|paulsen2002completely]] Lm.3.13）}，只要证明
$$
...
> [!corollary]
> - \label{cor:choimatrix}
>
$$
\mathcal{E}\text{全正}\Leftrightarrow (\mathcal{E} \otimes \mathbb{1})(|I\rangle\langle I|) \text{是正的}
$$
> [!proof]
...
>
$$
\begin{aligned} & (\mathcal{E} \otimes \mathbb{1})(|I\rangle\langle I|) \\ = & (\mathcal{E} \otimes \mathbb{1})\left(\sum_{i j}|i\rangle\langle j| \otimes|i\rangle\langle j|\right)  \\ = & (\mathcal{E} \otimes \mathbb{1})\left(\sum_{i j} E_{i j} \otimes E_{i j}\right)   \\ = & \left(\mathcal{E}\left(E_{i j}\right)\right)  \end{aligned}
$$
> [!note]
> - 通过这个推论可以容易地检验转置操作虽然是线性、保迹、正的，却不是全正的。

使用Choi-Jamiolkowski同构可以可以使把[[1 Stories of Bizzo's Life/5 量子科学和量子技术/量子信道\|量子信道]]从抽象的映射变成具体的矩阵，便于我们调动关于具体矩阵的性质和定理。设$\rho\in D(\mathsf{H})$是$d$阶密度矩阵，[[1 Stories of Bizzo's Life/5 量子科学和量子技术/量子信道\|量子信道]]$\mathcal{E}: B(\mathsf{H}^A) \rightarrow B(\mathsf{H}^B)$，以$\mathcal{E}$为元素的集合与$B(\mathsf{H}^B\otimes\mathsf{H}^{A*})$同构，按照以下方式构造一个具体的同构。设$\mathrm{dim}\mathsf{H}^A=d$，$\mathrm{dim}\mathsf{H}^B=d^\prime$，$\{|e_i\rangle_A\}$，$\{|e_i\rangle_{A^*}\}$和$\{|e_i^\prime\rangle_B\}$分别是$\mathsf{H}^A$，$\mathsf{H}^{A*}$和$\mathsf{H}^B$的一组ONB。称约化后得到最大混态（即正比于单位算符的态）的量子态为**最大纠缠态**(maximally entangled)，任意取定(未归一化的)最大纠缠态，
$$
|I\rangle=\sum_{i=1}^{d}|e_i\rangle_A| e_i\rangle_{A^*}
$$
> [!theorem] Choi-Jamiolkowski(CJ)同构
{ #th-th-cjinso}

> - 设线性映射$\mathcal{E}: B(\mathsf{H}^A) \rightarrow B(\mathsf{H}^B)$，则线性映射
>
$$
\mathsf{CJ}: \mathcal{E}\mapsto\rho_{B|A}^\mathcal{E}\equiv \mathsf{E}\equiv (\mathcal{E} \otimes \mathbb{1})(|I\rangle\langle I|) \in B(\mathsf{H}^B\otimes\mathsf{H}^{A*}),~    |I\rangle=\sum_{i=1}^{d}|e_i\rangle_A| e_i\rangle_{A^*}
$$
>
>  
{ #eq-eq-choiiso-1}
...
>
$$
\mathsf{CJ}: \mathcal{E}\mapsto\rho_{B|A}^\mathcal{E}\equiv \mathsf{E}\equiv (\mathbb{1} \otimes\mathcal{E} )(|I\rangle\langle I|) \in B(\mathsf{H}^{A*}\otimes\mathsf{H}^B),~|I\rangle=\sum_{i=1}^{d}|e_i\rangle_{A^*}| e_i\rangle_{A}
$$
>
>  
{ #eq-eq-choiiso-2}
...
>
$$
\mathsf{CJ}:\mathsf{Chn}(D(\mathsf{H}^A),D(\mathsf{H}^B))\rightarrow \mathsf{Choi}(\mathsf{H}^B\otimes \mathsf{H}^{A*})\equiv  \{\mathsf{E}\in B(\mathsf{H}^B\otimes \mathsf{H}^{A*}) \mid \mathsf{E}\geq 0,~\mathrm{tr}_B(\mathsf{E})=\mathbb{1}_{A^*} \}
$$
是信道的集合和满足保迹条件的态空间这两个凸空间之间的线性同构，  即$\mathcal{E}$是信道当且仅当$\mathsf{E}$是一个[[蔡氏矩阵\|蔡氏矩阵]]，称$\mathsf{E}$是$\mathcal{E}$的蔡氏矩阵，又称式\eqref{eq:choiiso_1}定义了**Choi-Jamiolkowski同构**。


> [!proof]
> - 同构只要证明逆射存在，参见命题\ref{prop:inverseofcj}。推论\ref{cor:choimatrix}已证，$\mathcal{E}$全正当且仅当$\mathsf{E}$半正定。$\mathcal{E}$保迹当且仅当$\mathrm{tr}_B(\mathsf{E})=\mathbb{1}_{A^*}$，参见命题\ref{prop:choiandtracep}。

关于蔡氏同构的满射的证明，参见[[Fundamental Limitations on the Recoverability of Quantum Processes\|文献]]式(12)。如果将定义蔡氏同构中的最大纠缠态改为约化满秩态，会得到另一个不一样的映射，它仍然是一个同构（[[Fundamental Limitations on the Recoverability of Quantum Processes\|文献]]式(28)）。这会导致不一样的蔡氏矩阵，文献中还考虑了他们之间的关系

> [!note]
> - 有时候也使用定义$\mathsf{E}=(\mathcal{E} \otimes \mathbb{1})(|I\rangle\langle I|)/d$，不要随意切换！

> [!definition] 蔡氏矩阵
> - 设矩阵$\mathsf{E}:B(\mathsf{H}^B\otimes\mathsf{H}^{A*})$满足，$\mathsf{E}\geqslant 0$以及$\mathrm{tr}_B(\mathsf{E})=\mathbb{1}_{A^*}$，则称$\mathsf{E}$为**蔡氏矩阵**。

> [!example] ~蔡氏矩阵
> -
> - 求迹信道的蔡氏矩阵$\mathrm{tr}:\mathsf{H}\rightarrow \mathbb{C}$，$\rho^\mathrm{tr}_{\mathbb{C}|\mathsf{H}}=\mathrm{tr}\otimes \mathrm{id}(|I\rangle\langle I|)=\mathbb{1}_{\mathsf{H}^*}$
...
$$
\mathrm{tr}_B(\mathsf{CJ}\circ \mathsf{CJ}^{\prime -1}(\mathsf{F}))=\mathrm{tr}_B(U_{BA}^\dagger \mathsf{F} U_{BA})=\mathrm{tr}_B(U_{BA}^\dagger \sum X_i\otimes Y_i B_{BA})=U_A^\dagger \sum \mathrm{tr}(X_i)Y_i U_A^\dagger = U_A^\dagger \mathbb{1}_{A^*} U_A=\mathbb{1}_{A^*}
$$
...
> [!theorem] 蔡氏矩阵的唯一性
> - 设$U\in B(\mathsf{H}^B\otimes \mathsf{H}^{A*})$是幺正变换，则$T=\{\mathsf{E}\in B(\mathsf{H}^B\otimes \mathsf{H}^{A*})\mid \mathrm{tr}_B(\mathsf{E})=\mathbb{1}_{A^*}  \}$，则
>
$$
U T U^\dagger = T \implies U=U_1\otimes U_2
$$
> [!proof]
> - 设$\sigma_i^A$和$\sigma_i^B$是两个系统上的无迹厄米算符基底，则$T$中的一般的算符可以写成{（[[oreshkov2012QuantumCorrelationsNo\|oreshkov2012QuantumCorrelationsNo]] S8）}
>
$$
T=\frac{I}{d_{B}}\left(\mathbb{1}+\sum_{i>0} a_i \sigma_i^{B}+\sum_{i j>0} t_{i j} \sigma_i^{A} \sigma_j^{B}\right)
$$
>
>
$$
...
$$
>   而$UTU^\dagger$等价于$U T^\perp U_\dagger$，根据引理\ref{lm:localuiffshu1}，后者蕴含$U$是局域的。

> [!proposition] Choi-Jamiolkowski(CJ)同构的逆{\cite[p.475
> - {Encyclopedia2006}},{（[[milz2021\|milz2021]] (95)）}]\label{prop:inverseofcj}
...
= & \left.\left.\left.\operatorname{tr}_{A^* A}\left((\mathcal{E}( | i\rangle_A\langle i|\right) \otimes|i\rangle_{A^*}\langle j| \otimes \mathbb{1}_A\right)\left(\mathbb{1}_B \otimes | k\right\rangle_{A^*}\langle l| \otimes|k\rangle_A\langle l|\right)\left(\mathbb{1}_A \otimes \mathbb{1}_{A^*} \otimes \rho_A\right)\right) \\
...
\begin{aligned}
& \operatorname{tr}_{A^* A}\left(\rho_{B | A}^{\mathcal{E}} \tau_A^{\mathrm{id}} \rho_A\right)  \\
& \left.\left.=\operatorname{tr}_{A^* A}\left((\mathcal{E}( | i\rangle_A\langle j|\right) \otimes|i\rangle_{A^*}\langle j| \otimes \mathbb{1}_A\right)\left(\mathbb{1}_B \otimes|k\rangle_{A^*}\langle l| \otimes|k\rangle_A\langle l|\right)\left(\mathbb{1}_B \otimes \mathbb{1}_{A^*} \otimes \rho_A\right)\right) \\
& \left.=\operatorname{tr}_{A^* A}\left(\rho_{B | A}^{\mathcal{E}} \otimes \mathbb{1}_A\left(\mathbb{1}_B \otimes | k\right\rangle_{A^*}\langle l| \otimes|k\rangle_A\langle 1|\right)\left(\mathbb{1}_B \otimes \mathbb{1}_{A^*} \otimes \rho_A\right)\right) \\
& \left.=\operatorname{tr}_{A^*}\left(\rho_{B | A}^{\mathcal{E}}\left(\mathbb{1}_B |k\right\rangle_{A ^*}\langle l|\right)\langle l| \rho_A|k\rangle\right) \\
& =\operatorname{tr}_{A^*}\left(\rho_{B | A}^{\mathcal{E}}\left(\mathbb{1}_B \otimes \rho_A^T\right)\right) \\
...
>
>   对偶基的使用是合理的，这是因为转置(式\eqref{label})被定义为$\rho_A^\top: \mathsf{H}^{A*}\rightarrow \mathsf{H}^{A*}$，转置且与$B$直积后的$\rho^A$与$\mathsf{E}$形状保持一致。
>
>   注意到性质$\mathrm{tr}_2((\mathbb{1}_1\otimes B)( C\otimes D))=\mathrm{tr}_2((C\otimes D)(\mathbb{1}_1 \otimes B) )$，所以有些步骤中$\mathsf{E}$的位置是可以变动的。
>
...
$$
>
>   利用式\eqref{eq:inverchoiiso}中的公式，可以重新计算出蔡氏矩阵，
>
$$
\begin{aligned} & (\mathcal{E} \otimes \mathbb{1})|I \rangle\langle I| \\ = & (\mathcal{E} \otimes \mathbb{1})\left(\sum\left|e_{k}\right\rangle\left\langle e_{l}|\otimes| e_{k}\right\rangle\left\langle e_{l}\right|\right)   \\ = & \sum \operatorname{tr}\left(\mathsf{E}\left( |e_{i}^{\prime} \rangle \langle e_{j}^{\prime}|\otimes| e_{l} \rangle \langle e_{k} |\right)\right)\left|e_{j}^{\prime}\right\rangle\left\langle e_{i}^{\prime}|\otimes| e_{k}\right\rangle\left\langle e_{l}\right| \\ = & ~\mathsf{E}\end{aligned}
$$
> [!proposition] 用蔡氏矩阵刻画保迹性和unitality
> - \label{prop:choiandtracep}
>   映射$\mathcal{E}$保迹当且仅当$\mathrm{tr}(\mathcal{E}(|i\rangle\langle j|))\otimes|i\rangle\langle j|=\delta_{ij}|i\rangle\langle j|=\mathbb{1}_{A^*}$，或等价地$\mathrm{tr}_B(\mathsf{E})=\mathbb{1}_{A^*}$。
>
...
$$
>   其中${}_i \mathsf{E}=\mathrm{tr}_i(\mathsf{E})\otimes \mathbb{1}_i/d_i$是子系统$i$的完全退极化信道(定义\ref{def:completelydepolarizing})，可以证明$\mathcal{P}$是一个算符系统上的正交投影(定义\ref{def:projectchannelof})，则
>
$$
\mathrm{tr}_B(\mathsf{E})=\mathbb{1}_{A^*}\Leftrightarrow \mathrm{tr}(\mathsf{E})=d_A\text{ 且 } \mathrm{tr}_B(\mathsf{E})=\mathrm{tr}(\mathsf{E})\otimes  \mathbb{1}_{A*}/d_A\Leftrightarrow  \mathrm{tr}(\mathsf{E})=d_A\text{ 且 } \mathcal{P}\mathsf{E}=\mathsf{E}
$$
>
>
>
>   映射$\mathcal{E}$是unital的当且仅当$\mathrm{tr}_{A^*}(\mathsf{E})=\mathbb{1}_B$

> [!proof]
> - 保迹条件的投影算符刻画的证明参见{（[[hoffreumon2021MultiroundProcessMatrix\|hoffreumon2021MultiroundProcessMatrix]] Apd.2）},{（[[hoffreumon2023CharacterizationHigherorderQuantum\|hoffreumon2023CharacterizationHigherorderQuantum]] (3.10)）}

> [!note]
> - 综合该命题和推论\ref{cor:choimatrix}，可得信道的蔡氏矩阵刻画。
...
直接将上述分解式代入\eqref{eq:inverchoiiso}可得
$$
\begin{aligned}
    \mathrm{tr}_2\left((\mathbb{1}_1\otimes (\cdot)^\top)\mathsf{E}\right)=\mathrm{tr}_2\left((\mathbb{1}\otimes \rho^\top) \sum_j \mathcal{E}(\varrho_j)\otimes \vartheta_j^*\right)=\sum \mathcal{E}(\varrho_j)\mathrm{tr}(\rho^\top \vartheta_j^*)=\sum \mathcal{E}(\varrho_j)\mathrm{tr}(\rho\vartheta_j^\dagger)=\mathcal{E}(\rho)
\end{aligned}
$$
这证明了分解式\eqref{eq:decompchoi}的正确性。
...
蔡氏矩阵
$$
\begin{aligned}\mathsf{E}=\mathcal{E}\otimes \mathbb{1}(|I\rangle\langle I|) &=\sum E_{k}|i\rangle \langle j|E_{k}^{\dagger}\otimes|i\rangle \langle j|\\ &=\sum|l\rangle \langle l|E_{k}|i\rangle \langle j|E_{k}^{\dagger}|m\rangle\langle  m|\otimes|i\rangle \langle j|\\ &=\sum\langle l|E_{k}|i\rangle |\ell\rangle |i\rangle \langle m|\langle j|\langle j|E_{k}^{\dagger}|m\rangle \\ &=\sum|E_{k}\rangle \langle E_{k}|\in B(\mathsf{H}^B\otimes \mathsf{H}^{A^*})
    \end{aligned}
$${ #eq-eq-krauschoimatrix}


> [!proposition] 复合信道的蔡氏矩阵[[chiribella2009TheoreticalFrameworkQuantum\|chiribella2009TheoreticalFrameworkQuantum]],{\cite[(187)
> - {milz2021}}]\label{prop:compochannalchoi}
...
>
$$
\mathsf{K}\star\mathsf{M} =\mathrm{tr}_{2^*}\left((\mathbb{1}_3\otimes \mathsf{M}^{\top_2} )(\mathsf{K}\otimes \mathbb{1}_1)\right) =\mathrm{tr}_{2^*}(\mathsf{M}^{\top_2}\mathsf{K})=\mathrm{tr}_{2^*,2}\left(\mathsf{K}\tau_2^\mathrm{id}\mathsf{M}\right)
$$
>   其中$\mathsf{K}\star \mathsf{M}\in B(\mathsf{H}_3\otimes \mathsf{H}_1^*)$，$\tau_2^{\mathrm{id}}:=\sum_{i, j}|i\rangle_{2^*}\langle j| \otimes|i\rangle_2\langle j| = |\mathbb{1}\rangle\langle \mathbb{1}|_{2^*2}$。
>
>   稍作推广，信道$\mathcal{HKM}$的蔡氏矩阵是
>
$$
\mathrm{tr}_{{2^*,3^*}}\left(\left(\mathbb{1}_4 \otimes \mathbb{1}_{3^*} \otimes \mathsf{M}^{\top_2}\right)\left(\mathbb{1}_4 \otimes \mathsf{K}^{\top_3} \otimes \mathbb{1}_{1^*}\right)\left(\mathsf{H} \otimes \mathbb{1}_{2^*} \otimes \mathbb{1}_{1^*}\right)\right)=\mathrm{tr}_{2^*,3^*}(\mathsf{M}^{\top_2}\mathsf{K}^{\top_3}\mathsf{H} )=\mathrm{tr}_{2^*,2,3^*,3}(\mathsf{H}\tau_C^{\mathrm{id}}\mathsf{K}\tau_B^\mathrm{id}\mathsf{M})
$$
...
>
$$
\mathsf{K}\star\mathsf{M} =\mathrm{tr}_{2^*}\left((\mathsf{M}^{\top_2} \otimes  \mathbb{1}_3)(\mathbb{1}_1 \otimes\mathsf{K} )\right) =\mathrm{tr}_{2^*}(\mathsf{M}^{\top_2}\mathsf{K})=\mathrm{tr}_{2^*,2}\left(\mathsf{K}\tau_2^\mathrm{id}\mathsf{M}\right)
$$
>   其中$\mathsf{K}\star \mathsf{M}\in B(\mathsf{H}_1^*\otimes \mathsf{H}_3)$，$\tau_2^{\mathrm{id}}:=\sum_{i, j}|i\rangle_{2}\langle j| \otimes|i\rangle_{2^*}\langle j| = |\mathbb{1}\rangle\langle \mathbb{1}|_{2^*2}$。
>
>   稍作推广，信道$\mathcal{HKM}$的蔡氏矩阵是
>
$$
\mathrm{tr}_{{2^*,3^*}}\left(\left(\mathsf{M}^{\top_2}\otimes \mathbb{1}_{3^*} \otimes \mathbb{1}_4 \right)\left(\mathbb{1}_{1^*} \otimes \mathsf{K}^{\top_3} \otimes \mathbb{1}_4\right)\left(\mathbb{1}_{1^*} \otimes \mathbb{1}_{2^*} \otimes \mathsf{H}  \right)\right)=\mathrm{tr}_{2^*,3^*}(\mathsf{M}^{\top_2}\mathsf{K}^{\top_3}\mathsf{H} )=\mathrm{tr}_{2^*,2,3^*,3}(\mathsf{H}\tau_C^{\mathrm{id}}\mathsf{K}\tau_B^\mathrm{id}\mathsf{M})
$$
...
$$
\begin{aligned}
\mathcal{K} \circ \mathcal{M}(\rho) & =\operatorname{tr}_2\left(\left(\mathbb{1}_{H_3} \otimes\left(\operatorname{tr}_{H_1}\left(\left(\mathbb{1}_{H_2} \otimes \rho^{\top}\right) \mathsf{M}\right)\right)^{\top}\right) \mathsf{K}\right) \\
& =\operatorname{tr}_{2}\left(\mathbb{1}_3 \otimes \operatorname{tr}_1\left(\left(\mathbb{1}_2 \otimes \rho^{\top}\right) \mathsf{M}^{T_2}\right) \mathsf{K}\right) \\
& =\operatorname{tr}_{12}\left(\left(\mathbb{1}_3 \otimes \mathbb{1}_2 \otimes \rho^{\top}\right)\left(\mathbb{1}_3 \otimes \mathsf{M}^{T_2}\right)\left(\mathsf{K} \otimes \mathbb{1}_1\right)\right) \\
& =\operatorname{tr}_1\left(\mathbb{1}_3 \otimes \rho^{\top} \operatorname{tr}_2\left(\mathbb{1}_3 \otimes \mathsf{M}^{T_2}\right)\left(\mathsf{K} \otimes \mathbb{1}_1\right)\right) \\
\mathcal{K} \mathcal{M}(\rho) & =\operatorname{tr}_1\left(\left(\mathbb{1}_3 \otimes \rho^{\top}\right) \mathsf{K} \star \mathsf{M}\right)
\end{aligned}
$$
>
>   利用式\eqref{eq:inverchoiiso}中的另一个公式可得
>
$$
\begin{aligned}
...
遵循类似的推导，如果我们总是将偏迹外面的转置完整地作用到最内层括号，而不是将其视为部分转置，可以得到$P\left(\mathcal{M}_i^A, \mathcal{M}_j^B\right)=\operatorname{tr}\left[\mathcal{M}_i^A \circ \mathcal{C} \circ \mathcal{M}_j^B\left(\rho^{B_1}\right)\right]$对应的过程矩阵{（[[oreshkov2012QuantumCorrelationsNo\|oreshkov2012QuantumCorrelationsNo]] (5)）}为
$$
W^{A_2 A_1 B_2 B_1} = \mathbb{1}_4\otimes \mathsf{C}^\top \otimes \rho^\top
$$
过程矩阵满足
$$
...
如果使用相反的蔡氏矩阵的约定$\mathsf{E}=\mathbb{1}\otimes \mathcal{E}(|I\rangle \langle I|)$，那么
$$
\operatorname{tr}\left[\mathcal{M}_i^A \circ \mathcal{C} \circ \mathcal{M}_j^B\left(\rho^{B_1}\right)\right]=\mathrm{tr}\left( \rho^\top \otimes \mathsf{C}^\top\otimes \mathbb{1}_4 \mathsf{M}^B\otimes \mathsf{M}^A\right)
$$
与另一种约定的顺序刚好相反。

更一般的过程矩阵定义参见\ref{def:processmatrix}

\begin{figure}[htbp]
 \centering
 \includegraphics[width=0.7\textwidth]{image//2024-07-01-12-14-42.png}
 \caption{待整理：蔡氏矩阵和信道的双矢量化只差矩阵元素的重排 proj.p.71}
 \label{  }
\end{figure}

蔡氏矩阵可以进行概率性的信道传态，
$$
\begin{aligned}&\langle \mathbb{1} |_{23} \mathcal{E} \otimes \mathcal{I} (|\mathbb{1}\rangle\langle\mathbb{1}|)_{12} \otimes \rho_3 |\mathbb{1}\rangle_{23} \\
...
> [!theorem] 量子信道的Choi-Kraus算符和表示{}{} （[[beck2020localization\|beck2020localization]] Th.1.17）
{ #th-th-operatorsumkarus}
...
>
$$
\begin{aligned} \mathcal{E}(\rho) & =\mathcal{E}(|\psi\rangle\langle\psi|) =\sum_{k l} c_{k} \overline{c_{l}} \mathcal{E}(|k\rangle\langle l|) =\langle\bar{\psi}|(\mathcal{E} \otimes \mathbb{1})(|k\rangle\langle l|\otimes| k\rangle\langle l|)| \bar{\psi}\rangle \\ & =\langle\bar{\psi}|(\mathcal{E} \otimes \mathbb{1})(|I\rangle\langle I|)| \bar{\psi}\rangle d_{A}^{2} =\left\langle\psi \mid\left(\sum_{k} p_{k}\left|\varphi_{k}\right\rangle\left\langle\varphi_{k}\right|\right) \psi\right\rangle d_{A}^{2}  =\sum R_{k}|\psi\rangle\langle\psi| R_{k}^\text{} \\ & R_{k}|l\rangle  \equiv \sqrt{p_{k}} d_{A}\left\langle l \mid \varphi_{k}\right\rangle \\ \Rightarrow & R_{k}|\psi\rangle=\sqrt{p_{k}} d_{A}\left\langle\bar{\psi} \mid \varphi_{k}\right\rangle\end{aligned}
$$
>
>   (全正性$\Leftarrow$算符和形式)
>
$$
\mathcal{E}\otimes \mathrm{id}(|I\rangle\langle I|)=\sum_k E_k \otimes \mathbb{1}|I\rangle\langle I| E_k^{\dagger} \otimes \mathbb{1} \geq 0 .
$$
>   其中$|I\rangle =\sum |ii\rangle$是未归一化的最大纠缠态。

> [!note]
> - Kraus算符分解可以被限制到给定的密度算符$\rho$的支集上，有更好看的分解形式{（[[tyson2010TwosidedBoundsMinimumerror\|tyson2010TwosidedBoundsMinimumerror]] Def.41）}。
>
>   另一个证明参见\ref{eq:stinespringtokraus}。

\begin{mybox}{}{}
    量子信道$\mathcal{E}$存在
    “$\rho$-Kraus 分解”，取$\rho$为最大混态，该分解实际上要求Kraus算符具有正交归一性
    {（[[tyson2010TwosidedBoundsMinimumerror\|tyson2010TwosidedBoundsMinimumerror]] (140)）}
$$
\mathcal{E}(\cdot)=\sum p_i F_i\cdot F_i^\dagger
$$
其中$F_i$满足$\mathrm{tr}\left(F_k^\dagger F_l\right)=n\delta_{kl}$，计算该分解的具体方法是(\ldots)。
 \end{mybox}
 
 
Kraus算符有一定的规范自由度，用一个幺正算符中的元素对Kraus算符做线性组合，得到的Kraus算符对应相同的量子信道。
> [!proposition] 算符和表示的幺正自由度
> - 设 $\mathcal{E}:B(\mathsf{H}_1)\rightarrow B(\mathsf{H}_2)$是线性映射，满足$\mathcal{E}(\cdot)=\sum_{i=1}^n E_i\cdot E_i^\dagger$，则下列等价
>   \begin{enumerate}[(1)]
> - $\mathcal{E}(\cdot)=\sum_{i=1}^n F_i\cdot F_i^\dagger$
> - 存在$n\times n$的幺正矩阵$(u_{ij})$，满足$E_{i}=\sum_{j} u_{i j} F_{j}$
>   \end{enumerate}
...
$$
>   设矩阵\footnote{终于有了令人愉悦的矩阵写法。}$A=(\alpha_{ij})$，则
>
$$
...
$$
>   其中，不等号是由于$A$的列数为$d^2$，最后一个等号可以用奇异值分解或者讨论方程$AX=0$的解空间的维度来证明，下面使用第二种方法，只要证明关于$X$的方程$AX=0$和$A^\text{} AX=0$同解即可，从$A^\text{} AX=0$可知$(AX)^\text{} AX=0$，因此$AX=0$，两个方程的解空间相同。设$S$是$n$元方程组$AX=0$的解空间，根据定理{（[[李忠华线代\|李忠华线代]] Th.5.33）}
$$
\mathrm{dim}S=n-\mathrm{rank}A
$$
可知
$$
\mathrm{rank}(AA^\text{})=\mathrm{rank}(A)
$$
>
...
>
>
>   根据内积$\langle A,B\rangle=\mathrm{tr}(A^\text{} B )$的正定性可知，$\{F_i\}_{i=1,\ldots,M}$中的前$r$个非零，第$r+1$至第$M$个为零。由反证法可知，不可能存在项数更少的Kraus算符和表示，假设存在个数少于$r$个的$\mathcal{E}$的算符和表示$\{G_i\}$，满足
>
$$
G_{i}=\sum_{j=1}^{M} \overline{v_{i j}}E_{j}
$$
>   重复式\eqref{eq:wtrans}中的计算过程可知，$W$与一个秩小于$r$的矩阵相似，秩也小于$r$，与假设矛盾，因此Kraus算符至少有$r$个。

> [!note]
> - 本例使用的是幺正相似对角化而不是复合同对角化。
>
>   通常称$W$的秩$r$为量子信道~$\mathcal{E}$的**秩**[[Fujiwara2008\|Fujiwara2008]]，也称为**蔡氏秩**{（[[watrous2018TheoryQuantumInformationa\|watrous2018TheoryQuantumInformationa]] p.78）}或**Kraus秩**[[wolf2008DividingQuantumChannels,kraus1983CoexistentEffectsObservables\|wolf2008DividingQuantumChannels,kraus1983CoexistentEffectsObservables]]。显然，信道的秩等于蔡氏矩阵的秩：
>
$$
\begin{aligned}
& \sum \mathcal{E}\otimes \mathbb{1}(|I\rangle\langle I|) \\
= & \sum E_i \otimes \mathbb{1}|I\rangle\langle I| E_i^{\dagger} \otimes \mathbb{1} \\
...
$$
>
>   这组ONB满足\footnote{此式没有提供证明，但是经过程序验证，参见：基本矩阵作为基底.nb。}
>
$$
...
$$
>   上式的右侧不能简单地写成两个矩阵的乘积，显然，在一般情况下\footnote{例如：$A$是一个行向量，$W=A^*A^\intercal$的秩为1，但是可以选取$A$使得$\widetilde{\mathcal{E}}$秩大于1。具体的数值举例，参见：quantumChannel的秩举例.nb}。
>
$$
\mathrm{rank}(\mathcal{\widetilde{E}})\neq \mathrm{rank}(A^\intercal \overline{A})=\mathrm{rank}(\overline{A}A^\intercal)=r
$$
>   进而，我们不可能找到一组ONB，使得 $\operatorname{tr}\left(e_{l}^{\text{}} e_{p} e_{i} e_{q}^{\text{}}\right)=\delta_{lp}\delta_{iq}$成立\footnote{有一组基底拥有非常相似的性质：[Sylvester's generalized Pauli matrices](https://www.wikiwand.com/en/Generalizations_of_Pauli_matrices)}，否则对于任意的$\mathcal{E}$，都有$\mathrm{rank}(\widetilde{\mathcal{E}})=r$成立。

> [!proposition] 量子操作的性质
> - 设$\mathcal{K}:D(\mathsf{H})\rightarrow D_\leq(\mathsf{H})$是量子操作，$\{K_i\}_{i=0,\ldots, m}$是相应的Kraus算符，那么对任意的$0<\kappa\leq 1$，有
>
$$
\sum_{l=1}^{m} K_{l}^{\text{}} K_{l} \leq \kappa \mathbb{1} \quad \Leftrightarrow \quad \operatorname{tr}(\mathcal{K}(\rho)) \leq \kappa \quad \forall \rho \in D(\mathsf{H})
$$
>   以及
>
$$
\sum_{l=1}^{m} K_{l}^{\text{}} K_{l} =\kappa \mathbb{1} \quad \Leftrightarrow \quad \operatorname{tr}(\mathcal{K}(\rho)) = \kappa \quad \forall \rho \in D(\mathsf{H})
$$
> [!proof]
> - 首先注意到$\mathcal{K}(\rho)$和$\sum_{l=1}^{m} K_{l}^{\text{}} K_{l}$是正定的。
>
>   (必要性) $\sum_{l=1}^{m} K_{l}^{\text{}} K_{l} \leq \kappa \mathbb{1}$意味着$\sum_{l=1}^{m} K_{l}^{\text{}} K_{l}$的最大本征值小于等于$\kappa$，利用谱分解，有
>
$$
...
> [!theorem] Kraus算符形式与系统环境耦合形式的等价性{} （[[Scherer2019\|Scherer2019]] Th.3.24）
{ #th-th-krausancilla}

> - $\mathcal{K}:B(\mathsf{H_1})\rightarrow B(\mathsf{H_1})$是线性映射，$\rho_1\in D(\mathsf{H_1})$则任取$0<\kappa\leqslant 1$，下列等价
>   \begin{enumerate}[(1)]
> - 存在Hilbert空间$\mathsf{H}_2$，线性变换$V\in B(\mathsf{H_1}\otimes \mathsf{H_2})$和量子态$\rho_2\in D( \mathsf{H_2})$，使得
>
$$
V^\dagger V\leqslant \kappa \mathbb{1}
$$
>
$$
\mathcal{K}(\rho_1)=\mathrm{tr}_2(V\rho_1\otimes \rho_2 V^\dagger)
$$
> - 存在一组Kraus算符$K_l\in B(\mathsf{H_1})$，满足
>
$$
\sum_{l=1}^{m} K_l^\dagger K_l\leqslant \kappa \mathbb{1}
$$
>
$$
\mathcal{K}(\rho_1)=\sum_{l=1}^{m}K_l\rho_1K_l^\dagger
$$
> - 存在线性变换$W:B(\mathsf{H}_1,\mathsf{H_1}\otimes \mathsf{H_2})$，使得
>
$$
W^\dagger W\leqslant \kappa \mathbb{1}
$$
>
$$
...
>   另外
$$
W^\dagger W=\kappa \mathbb{1}\Leftrightarrow V^\dagger V=\kappa \mathbb{1}\Leftrightarrow \sum_{l=1}^{m}K_l^\dagger K_l=\kappa \mathbb{1}
$$
> [!proof]
...
>
$$
\tilde{V}\left(e_{1} f_{1}, \cdots, e_{n} f_{1}\right)=\left(e_{1} f_{1}, \cdots, e_{n} f_{n}\right)(\tilde{V})_{n^{2} \times n}
$$
>
$$
\begin{aligned} \forall|\psi\rangle, & \left\langle\psi\left|\left\langle f_{1}\left|\tilde{V}^\dagger \tilde{V}\right| \psi\right\rangle\right| f_{1}\right\rangle=\sum\left\langle\psi\left|K_{l}{ }^\dagger K_{l}\right| \psi\right\rangle \leqslant 1 \\ \Rightarrow & \tilde{V}^\dagger \tilde{V} \leqslant \mathbb{1}_{\iota \left(H_{1}\right)}\end{aligned}
$$
>   $\tilde{V}$只定义在了某个子空间上，为了进一步得到$V$，需要做合适的拓展
>
$$
\begin{array}{l}V^\dagger V=\left(\begin{array}{l}\tilde{V}^\dagger \\ A^\dagger\end{array}\right)(\tilde{V}~~ A)=\left(\begin{array}{ll}\tilde{V}^\dagger \tilde{V} & \tilde{V}^\dagger A \\ A^\dagger \tilde{V} & A^\dagger A\end{array}\right) \\ \left\{\begin{array}{l}\tilde{V}^\dagger A=0 \\ A^\dagger A=\mathbb{1} .\end{array}\right.\end{array}
$$
>   由此定义的$V$满足$V^\dagger V \leqslant \mathbb{1}$，以及
>
>
$$
\begin{aligned}&\operatorname{tr}_{2}\left( {V} |\psi\rangle\left\langle\psi|\otimes| f_{1}\right\rangle\left\langle f_{1}\right| V^\dagger\right) \\ =&\operatorname{tr}_{2}\left(\widetilde{V}|\psi\rangle\left\langle\psi|\otimes| f_{1}\right\rangle\left\langle f_{1}\right| \widetilde{V}^\dagger\right) \\ =&\operatorname{tr}_{2}\left(\sum_{l} K_{l}|\psi\rangle \langle\psi |K_{l}^\dagger | f_{1} \rangle\left\langle f_{1}\right|\right) \\ =&\sum_{l} K_{l}|\psi\rangle\langle\psi| K_{l}^\dagger \\ =& \mathcal{K}(|\psi\rangle\langle\psi|)\end{aligned}
$$
>
>   (1)$\Leftrightarrow$(3)显然。

> [!note]
> - 系统环境耦合形式的量子信道表明，任何量子信道都可以通过一个直积系统上的幺正变换来实现，为量子信道提供了一种物理上的实现方式。然而这种表示信道方式的缺点是辅助系统的初态$\rho_2$和幺正变换$V$都具有较大的自由度。最小表示在不计环境的幺正变换的意义下是唯一的{（[[kretschmann2006InformationDisturbanceTradeoffContinuity\|kretschmann2006InformationDisturbanceTradeoffContinuity]] (12)）}。
>
>   实际上$V$有一个显式解[[lei2024DilationDiscriminationUhlmanns\|lei2024DilationDiscriminationUhlmanns]]。
>
>   如果没有直积态的条件就不能写成Kraus算符和的形式、也不能被视为量子信道。
>
>   从原信道到对偶信道的系统环境耦合表示（[[serafini2017QuantumContinuousVariablesa\|serafini2017QuantumContinuousVariablesa]] (5.61)）：
>
$$
...
$$
\begin{aligned}
\operatorname{Tr}_1\left[\hat{A} \Phi^*(\hat{B})\right] & =\operatorname{Tr}_1[\Phi(\hat{A}) \hat{B}]=\operatorname{Tr}_{1,2}\left[\hat{S}^{\dagger}(\hat{A} \otimes|0\rangle\langle 0|) \hat{S}(\hat{B} \otimes \hat{\mathbb{1}})\right] \\
& =\operatorname{Tr}_1\left[\hat{A} \operatorname{Tr}_2\left[(\hat{\mathbb{1}} \otimes|0\rangle\langle 0|) \hat{S}(\hat{B} \otimes \hat{\mathbb{1}}) \hat{S}^{\dagger}\right]\right],
\end{aligned}
$$
>   上式中的$\hat{A}$是任意的，因此有对偶信道的系统环境耦合表示
>
$$
\Phi^*(\hat{B})=\operatorname{Tr}_2\left[(\hat{\mathbb{1}} \otimes|0\rangle\langle 0|) \hat{S}(\hat{B} \otimes \hat{\mathbb{1}}) \hat{S}^{\dagger}\right] .
$$
> [!example] 量子信道和量子操作
> -
> - 幺正演化$\mathcal{E}(\rho)=U \rho U^{\text{}}$、测量$\mathcal{E}_{m}(\rho)=M_{m} \rho M_{m}^{\text{}}$
> - Unruh信道[[banerjee2017CharacterizationUnruhChannel\|banerjee2017CharacterizationUnruhChannel]]。
> - 比特翻转、相位反转、比特相位翻转、恒等信道和退极化(depolarizing)、相位阻尼(退相位dephasing)、振幅阻尼(AD)\footnote{参见：几种量子信道quantum channel，信道-Kraus算符-蔡氏矩阵.nb}、自发辐射(Kraus算符将激发态打到基态)。恒等信道的蔡氏矩阵是最大纠缠态本身、完全退极化信道(定义\ref{def:completelydepolarizing})的蔡氏矩阵正比于单位阵。
> - 迹和偏迹。
> - $\mathcal{E}$ 是**unital 信道**$\Leftrightarrow~\mathcal{E}(\mathbb{1})=\mathbb{1}$$\Leftrightarrow \mathcal{E}^\dagger$是量子信道(保迹)。$\mathcal{L}(X)=0,~\forall X\iff \mathcal{L}^\dagger(\mathbb{1})=0$。
- 反例：部分转置
$$
...
$$
因此该量子信道不是单射，不具有可逆性。再比如求迹也是一个不可逆的量子信道。另外，即便对于可逆的量子信道，量子信道的逆也未必是量子信道，实际上，利用Wigner定理\footnote{Hilbert空间上的Wigner对称性操作都是幺正或者反幺正的{（[[Moretti2017\|Moretti2017]] Th.12.11）}}可以证明，量子信道的逆是量子信道当且仅当它是幺正的{（[[rivas2012book\|rivas2012book]] Th.3.4.1,Prop.12.13）}。

 \begin{figure}[!htb]
        \centering
        \includegraphics[width=0.99\linewidth]{image//algebra1.pdf}
        \caption{待整理：单比特的量子信道是Bloch球上的仿射变换}
        \label{fig:image1}
    \end{figure}
    

### 利用过程层析计算量子信道的Kraus算符
态、过程层析：可以通过获得一组信息完备的POVM的概率分布来确定量子态或者量子信道{（[[milz2021\|milz2021]] (86)）}（待补充）。
...
$$
由此可得与\eqref{eq:tensoreq}等价的压平后的方程
$$
...
其中，$m={(l-1)d^2+p},n=(j-1)d^2+k,$~$\lambda_{(l-1)d^2+p}^{\Finv }=\lambda_{lp},~\beta_{(l-1)d^2+p,(j-1)d^2+k }^{\Finv}=\beta_{jlkp}$。向量$\lambda_{m}^{\Finv }$和$\chi_{n}^{\Finv }$是矩阵压平的结果，而$\beta_{m n}^{\Finv}$是张量张量转置\footnote{4阶置换群的“置换形式”群元$(2,1,4,3)$表示被作用的列表中的第$1,2,3,4$个元素在置换后变为第$2,1,4,3$个元素，写成“循环形式”是$c(1,2)\circ c(3,4)$。利用PermutationCycles和PermutationList可以在置换形式和循环形式之间相互转换。}$\beta_{jlkp}\rightarrow \tilde{\beta}_{jlkp}=(2,1,4,3)\circ \beta_{jlkp}=\beta_{ljpk}$和数组压平的结果，即
$$
\beta_{(l-1)d^2+p,(j-1)d^2+k }^{\Finv }=\mathrm{Ft}(\tilde{\beta})_{(l-1)d^2+p,(j-1)d^2+k }=\tilde{\beta}_{ljpk} =\beta_{jlkp}
$${ #eq-eq-transflat}


上式已经过检验。最后，求解线性方程组\eqref{eq:flateq}，并对$\chi$做对角化，就可以求得$\mathcal{E}$的Kraus算符。式\eqref{eq:transflat}的验证，利用上述过程的算例参见文档\footnote{几种量子信道quantum channel，信道-Kraus算符-蔡氏矩阵.nb}。实际上，利用蔡氏矩阵与Kraus算符的关系式\ref{eq:krauschoimatrix}可以更简单地从蔡氏矩阵求解Kraus算符，幅值衰减的例子参见相同的文档。
 
### 量子信道的谱性质
谱分解类似第\ref{ssec:speclind}节，由于保厄米性，本征谱关于实数轴对称性，另外可以证明正且(保迹或unital)蕴含本征值都生活在单位圆盘上，即$|\lambda|\leq 1$，除了这两点以外就没有其他约束了{（[[wolf2012QuantumChannelsOperations\|wolf2012QuantumChannelsOperations]] chap.6）}。

> [!proposition] prop.6.2 [[wolfQuantumChannelsOperations\|wolfQuantumChannelsOperations]]
量子信道的模为1的本征值(边缘谱)都有平凡的Jordan块(代数重数等于几何重数)。

### 量子信道的不动点
\label{ssec:fixofqc}
每个保迹正映射，尤其是信道$\Phi$都有一个密度算符$\pi$作为不动点，即满足$\Phi(\pi)=\pi$ (证明详见[[watrous2018TheoryQuantumInformationa\|watrous2018TheoryQuantumInformationa]] Sec.4.2.2)，如果$\Phi$还是不可约的，那么任何不动点都正比于$\pi$[[lardizabal2022MeanHittingTime\|lardizabal2022MeanHittingTime]]。没法直接利用赋范空间和度量空间的不动点定理{（[[Moretti2017\|Moretti2017]] Th.2.111）}来证明(不动点的唯一存在性)是因为保迹正映射关于trace norm不是严格收缩的(参见定理\ref{th:prehpretiff})，而赋范空间中的不动点定理中要求严格收缩。

在Markov动力学下的不动点的唯一存在性的证明，参见[[yoshida2024UniquenessSteadyStates\|yoshida2024UniquenessSteadyStates]]
...
$$
### 量子信道的其他性质\label{sec:otherproperqc}


> [!definition] $\pi_{1,2}$协变量子信道
- \label{def:covariantchannel}
考虑群 $G$ 和它的两个(投影)幺正表示 $\pi_{1}, \pi_{2}$，表示空间记为$\mathsf{H}_{1}$ and $\mathsf{H}_{2}$ ，量子信道 $\mathcal{E}: {B}\left( \mathsf{H}_{1}\right) \rightarrow  {B}\left( \mathsf{H}_{2}\right)$ 称为**关于$\pi_{1,2}$协变的信道}(covariant with respect to $\pi_{1,2}$)或者**协变信道**，如果$\mathcal{E}$是等变映射(又称interwiner，定义\ref{def:equivariantfunc})，即
$$
   \begin{array}{l}
   \mathcal{E}\left[\pi_{1}(U) A \pi_{1}(U)^{*}\right]=\pi_{2}(U) \mathcal{E}[A] \pi_{2}(U)^{*} \\
   \forall A \in {B}\left( \mathsf{H}_{1}\right) \forall U \in G
  \end{array}

$$
如果两个表示都是不可约的，则称信道为**不可约协变信道**[[mozrzymas2017StructureIrreduciblyCovariant\|mozrzymas2017StructureIrreduciblyCovariant]]。

> [!note]
- 根据协变量子信道的定义，可以构造关于编码信道协变的量子纠错码，称为协变量子纠错码。
用范畴的语言，设$\mathsf{C}=\mathsf{B}G$，$D=\mathsf{Vect}$，则函子范畴\footnote{参见第\ref{ssec:cnf}节。}$[\mathsf{C},\mathsf{D}]$是以$G$的伴随表示为对象，以协变量子信道为态射的范畴。$\tilde{\pi}_{1,2}:\star \mapsto V_{1,2},~g\mapsto \pi_{1,2}(g) (\cdot )\pi_{1,2}(g)^*$是两个函子($[\mathsf{C},\mathsf{D}]$中的对象)，协变信道$\mathcal{E}$是$\pi_{1,2}$之间的自然变换，即满足$\mathcal{E}\circ \tilde{\pi}_1= \tilde{\pi}_2\circ\mathcal{E}$。

例子：
- $G=U(d)$，$\pi_{1}(U)=\pi_{2}(U)=U\in U(d)$，退极化信道
$$
   \mathcal{E}(\rho)=(1- p) \rho+p  \operatorname{tr}(\rho) \mathbb{1}/d

$$
是协变的，其中$0\leqslant p\leqslant \frac{d^2}{d^2-1}$保证了退极化信道是全正的。

> [!definition] 互补信道 (complementary) [[holevo2012QuantumChannelsTheir\|holevo2012QuantumChannelsTheir]]
- \label{def:compchannel}
...
> [!theorem] 利用互补信道刻画纠缠保真度{} （[[beny2010GeneralConditionsApproximate\|beny2010GeneralConditionsApproximate]] (Th.1)）
...
>
$$
其中$F_\mathrm{min}$是信道的纠缠保真度(定义\ref{eq:fidelity})。

> [!proof]
- \begin{figure}[htbp]
\centering
\includegraphics[width=0.7\textwidth]{image//2024-06-28-15-36-22.png}
\caption{  }
\label{  }
\end{figure}

> [!proposition] 投影Kraus算符蕴含投影信道
...
> [!proof]
- 参见\href{https://en.wikipedia.org/wiki/Holevo

> [!definition] 单次$\epsilon$-误码率的纠缠辅助的经典容量
-
$$
...
> [!definition] 可达码率和信道容量
- 一个码率是**可达的**(achievable)，如果任取$0<\varepsilon\leq 1$，$\delta>0$，存在$n$和一个$\left(n, 2^{n(R-\delta)}, \varepsilon\right)$协议。
信道的可达码率的上确界被称为**纠缠辅助经典通信的信道容量**（[[khatri2020PrinciplesQuantumCommunication\|khatri2020PrinciplesQuantumCommunication]] Def.11.12）
$$
C_{\mathrm{EA}}(\mathcal{N}):=\sup \{R: R \text { is an achievable rate for } \mathcal{N}\} .
$$

> [!theorem] 纠缠辅助的信道容量就是信道的互信息 （[[khatri2020PrinciplesQuantumCommunication\|khatri2020PrinciplesQuantumCommunication]] Th.11.16）
-
$$
...
## 量子通信简介
量子通信承诺可靠地传输量子信息，有效地分发纠缠和产生完全安全的密钥。在隐形传态任务中，如何A和B预先共享一个纠缠态，那么就可以通过经典通信的方式来实现任意量子比特的传输，由于噪声的存在共享后的纠缠态往往不是纯的，我们需要从大量不纯的纠缠态中蒸馏得到少量纯的纠缠态，以提高量子态的传输速率。在量子通信中，一个重要的任务是确定在无中继器且不限制局域操作和双向经典通信的情况下的点到点量子传输速率，根据目标态的不同，可以定义不同类型的量子容量，例如双向量子容量(two-way quantum capacity)[[pirandola2017FundamentalLimitsRepeaterlessa\|pirandola2017FundamentalLimitsRepeaterlessa]]。
## 信息论
### [[数据处理不等式\|数据处理不等式]]
任意态 $\rho$ 和 $\sigma$ 之间的量子相对熵在任意量子通道 $\mathcal{N}$ 的作用下是非增的，即：
$$
D(\rho \ || \ \sigma) – D(\mathcal{N}(\rho) \ || \ \mathcal{N}(\sigma)) \geq 0 \quad (2)
$$
更高阶的数据处理不等式的推广，参见[[Fundamental Limitations on the Recoverability of Quantum Processes\|文献]]。


利用数据处理不等式可以证明：$$\mathcal{N}(\rho)) \geq S(\rho)$$

**目标**：相对于单位阵的熵的负数就是自己的熵：$S(\rho) = -D(\rho \ || \ \mathbb{1})$，因此证明 $S(\mathcal{N}(\rho)) \geq S(\rho)$ 等价于证明 $D(\rho \ || \ \mathbb{1}) \geq D(\mathcal{N}(\rho) \ || \ \mathbb{1})$。
...
$$
\begin{align*}
D(\rho \ || \ \mathbb{1}) &\geq D(\mathcal{N}(\rho) \ || \ \mathcal{N}(\mathbb{1})) && \text{(1. 数据处理不等式)} \\
&\geq D(\mathcal{N}(\rho) \ || \ \mathbb{1}) && \text{(2. subunital与相对熵单调性)}
\end{align*}
$$

**步骤解释**：
1.  **数据处理不等式 (DPI)**：由于 $\mathcal{N}$ 是一个物理映射 (CPTP)，它不能增加输入态 $(\rho, \mathbb{1})$ 之间的可区分度。
2.  **亚幺正性 (Subunitality)**：$\mathcal{N}$ 的亚幺正性意味着 $\mathcal{N}(\mathbb{1}) \leq \mathbb{1}$。结合量子相对熵在其第二个参数上的单调性，即可得到此步。

**结论**：
从上述不等式链的头尾两端，我们直接得到 $D(\rho \ || \ \mathbb{1}) \geq D(\mathcal{N}(\rho) \ || \ \mathbb{1})$。两边乘以-1，即得证 $S(\rho) \leq S(\mathcal{N}(\rho))$。
### 经典信息与熵公式汇总
...
$$
- 量子信道的互信息（[[khatri2020PrinciplesQuantumCommunication\|khatri2020PrinciplesQuantumCommunication]] (7.11.102)）
$$
...
- 量子信道的经典容量（[[khatri2020PrinciplesQuantumCommunication\|khatri2020PrinciplesQuantumCommunication]] Th.12.3）
$$
C(\mathcal{N})=\chi_{\mathrm{reg}}(\mathcal{N}):=\lim _{n \rightarrow \infty} \frac{1}{n} \chi\left(\mathcal{N}^{\otimes n}\right) .
$$
- 量子信道的相干信息量（[[chiribella2021IndefiniteCausalOrder\|chiribella2021IndefiniteCausalOrder]] 4）,（[[khatri2020PrinciplesQuantumCommunication\|khatri2020PrinciplesQuantumCommunication]] 7.11.107）
$$
...
$$
上面的两个定义是否等价？
- 量子信道的量子容量（[[khatri2020PrinciplesQuantumCommunication\|khatri2020PrinciplesQuantumCommunication]] 14.0.1）
$$
