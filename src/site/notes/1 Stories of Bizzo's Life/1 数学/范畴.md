...
> [!note]
> - 对于态射 $f: X \rightarrow Y$, 若存在 $g: Y \rightarrow X$ 使得 $f g=\operatorname{id}_{Y}, g f=\operatorname{id}_{X}$, 则称 $f$ 是**同构** ，或称为**可逆**, 写作 $X \stackrel{f}{\cong} Y$ ， $g$ 称为 $f$ 的逆。Rotman将$f$称为范畴中的等价(equivalence)（[[rotman2013alto\|rotman2013alto]] p.12），函子保持等价。态射都是有方向的，不需要担心$f$和$g$之间的对易性。两条式子互不蕴含，为了证明同构，两者缺一不可。对象的“同构”是数学中两个对象是否在某种意义下“相等”的严格定义，只有态射才谈及“相等”，这个相等对于一些具体的例子来说是可以判断的，但是没有一般的方法来判断两个态射是否相等，在高阶范畴中，可以进一步谈论态射之间态射和“同构”。
>
...
在代数拓扑中，要定义**商范畴（Quotient Category）**，首先需要引入范畴上**同余Congruence**的概念。

**1. 同余（Congruence）的定义** 在一个范畴 $\mathscr{C}$ 中，同余 $\sim$ 是定义在 $\mathscr{C}$ 中所有态射（Morphisms）类上的一个等价关系 ，并且必须满足以下两个条件：

- **同源性**：如果 $f \in Hom(A, B)$ 且 $f \sim f'$，那么 $f'$ 也必须属于 $Hom(A, B)$（即等价的态射必须具有相同的定义域和目标域） 。
    
- **复合相容性**：如果 $f \sim f'$ 且 $g \sim g'$，并且它们的复合 $g \circ f$ 存在，那么必然有 $g \circ f \sim g' \circ f'$ 。
    

**2. 商范畴（Quotient Category）的构造** 设 $\mathscr{C}$ 是一个带有同余关系 $\sim$ 的范畴，用 $[f]$ 表示态射 $f$ 的等价类 。那么，$\mathscr{C}$ 关于 $\sim$ 的商范畴 $\mathscr{C}'$ 构造如下：

- **对象（Objects）**：商范畴 $\mathscr{C}'$ 的对象与原范畴 $\mathscr{C}$ 的对象完全相同，即 obj $\mathscr{C}'$ = obj $\mathscr{C}$ 。
    
- **态射（Morphisms）**：在 $\mathscr{C}'$ 中，对象 $A$ 到 $B$ 的态射集合 $Hom_{\mathscr{C}'}(A,B)$ 由原范畴中态射的等价类 $[f]$ 组成，其中 $f \in Hom_{\mathscr{C}}(A,B)$ 。这个态射集合通常被简记为 $[A, B]$ 。
    
- **态射的复合（Composition）**：商范畴中的复合运算通过态射的代表元来定义，即 $[g] \circ [f] = [g \circ f]$ 。
    

**典型例子** 在代数拓扑中，最重要也是最常用的商范畴例子是**同伦范畴（Homotopy Category，记作 hTop）** 。在同伦范畴中，对象是拓扑空间，而对象之间的态射不是简单的连续函数，而是连续函数的同伦等价类（即把相互同伦的连续函数视为同一个态射） 。
### 子范畴
...
*   **性质**：范畴的中心 $Z(\mathsf{C})$ 总是一个**交换幺半群 (commutative [[monoid\|monoid]])**。
### 例子
