---
{"dg-publish":true,"permalink":"/1 Stories of Bizzo's Life/1 数学/3 线性代数/","tags":["#线性代数"]}
---
...
简记为$\mathcal{A}(\alpha_j)=\beta_i a_{ij}$，如果$\alpha $是一组标准正交基，用braket符号记为$ $\mathcal{A} = a_{ij} |\beta_i\rangle \langle \alpha_j|$ $，如果$$\alpha$$不是标准正交基，则使用对偶基$$\alpha^*$，$\mathcal{A} = \sum_{i,j} a_{ij} |\beta_i\rangle \langle \alpha_j^*|$。
...
- [[1 Stories of Bizzo's Life/1 数学/1 群论和群表示论\|群论]] 
...
> [!definition] 半单代数
> 
> 非零有限维代数称为**半单**，如果 $\mathrm{rad} \mathfrak{A}=\{0\}$。

> [!note]
> Lie 代数的 radical 定义与代数的 radical 定义不同，而 Lie 代数的半单是用 Lie 代数的 radical 为零来定义的，因此 Lie 代数的半单与代数的半单不同，注意区分。
> 在[[群表示论#^f0x105\|群表示论#^f0x105]]中，有限维代数$A$是半单的当且仅当$A$的左正则模是完全可约的，即可以表示为单模的直和，这两个定义是等价的（待证）。
...
> [!note]
> 与定理 [[群表示论#^ys16wn\|群表示论#^ys16wn]] 相同。是否存在环同态的集合 $\mathrm{Hom}_\mathcal{A}(\mathfrak{G},\mathfrak{G})\cong e \mathfrak{A}e,~\theta_a \mapsto eae$，其中 $\theta_a(g)=ag$。
